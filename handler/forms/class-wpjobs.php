 ...
 * Wpjobs class.
 */
class Wpjobs extends \ChatOnDesk\FormInterface
{
     ...
     * @return stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::WP_JOB_MANAGER;
    ...
    public function newJobApplicationSendSms( $send_to, $job_id, $application_id )
    {
        ...
        $admin_number                         = chatondesk_get_option('sms_admin_phone', 'chatondesk_message', '');
        ...
        $msg_enable                           = get_option('chatondesk_sms_notification');
        $new_application_customer_msg_enable  = get_option('chatondesk_new_application_sms_status');
        $new_application_admin_msg_enable     = get_option('chatondesk_new_application_admin_sms_status');
        $new_application_candidate_msg_enable = get_option('chatondesk_new_application_candidate_sms_status');

        if ($msg_enable ) {
            $visitor_message   = get_option('chatondesk_new_application_sms');
            $admin_message     = get_option('chatondesk_new_application_admin_sms');
            $candidate_message = get_option('chatondesk_new_application_candidate_sms');
            ...
            if (! empty($visitor_message) && '1' === $new_application_customer_msg_enable ) {
                do_action('cod_send_sms', $phone, self::parseSmsContent($visitor_message, $datas));
            }

            if (! empty($candidate_message) && '1' === $new_application_candidate_msg_enable && ! empty($candidate_phone) ) {
                do_action('cod_send_sms', $candidate_phone, self::parseSmsContent($candidate_message, $datas));
            }

            if (! empty($admin_number) && ! empty($admin_message) && '1' === $new_application_admin_msg_enable ) {
                do_action('cod_send_sms', $admin_number, self::parseSmsContent($admin_message, $datas));
            }
        }
    }
    ...
    public function listingPublishedSendSms( $job_id )
    {
        ...
        $msg_enable                  = get_option('chatondesk_sms_notification');
        $approve_customer_msg_enable = get_option('chatondesk_job_approve_customer_sms_status');
        $approve_admin_msg_enable    = get_option('chatondesk_job_approve_sms_status');

        if ($msg_enable ) {
            $visitor_message = get_option('chatondesk_job_approve_customer_sms');
            $admin_message   = get_option('chatondesk_job_approve_sms');
    ...
    public function jobManagerSettings( $settings )
    {
        ...
        $data                   = array();
        $settings['smsalert'][] = 'Chat On Desk';

        $data[] = array(
        'name'     => 'chatondesk_sms_notification',
        'cb_label' => __('Enable to send sms notification to admin as well as employer', 'chat-on-desk'),
        'std'      => get_option('chatondesk_sms_notification', 1),
        'label'    => __('SMS Notification', 'chat-on-desk'),
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'     => 'chatondesk_new_job_sms_status',
        'cb_label' => __('Enable Admin Message When New Job Submitted', 'chat-on-desk'),
        'std'      => get_option('chatondesk_new_job_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'  => 'chatondesk_new_job_sms',
        'std'   => 'Dear admin, a new job [job_name] is submitted by [username].Please check your admin dashboard for complete details.',
        'label' => '',
        'desc'  => __('You can use following tokens [store_name], [job_id], [job_name] ,[username] ,[email] ,[phone]', 'chat-on-desk'),
        'type'  => 'textarea',
        );

        $data[] = array(
        'name'     => 'chatondesk_job_approve_sms_status',
        'cb_label' => __('Enable Admin Message When A Job Approved', 'chat-on-desk'),
        'std'      => get_option('chatondesk_job_approve_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'  => 'chatondesk_job_approve_sms',
        ...
        $data[] = array(
        'name'     => 'chatondesk_edit_job_sms_status',
        'cb_label' => __('Enable Admin Message When Job Edited', 'chat-on-desk'),
        'std'      => get_option('chatondesk_edit_job_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'  => 'chatondesk_edit_job_sms',
        ...
        $data[] = array(
        'name'     => 'chatondesk_new_application_admin_sms_status',
        'cb_label' => __('Enable Admin Message When New Application Submitted', 'chat-on-desk'),
        'std'      => get_option('chatondesk_new_application_admin_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'  => 'chatondesk_new_application_admin_sms',
        'std'   => 'Dear [username], a candidate [candidate_name] is applied for job.',
        'label' => '',
        'desc'  => __('You can use following tokens [store_name], [job_id], [job_name] ,[username] ,[email] ,[phone] ,[candidate_name] ,[candidate_email]', 'chat-on-desk'),
        'type'  => 'textarea',
        );

        $data[] = array(
        'name'     => 'chatondesk_new_job_customer_sms_status',
        'cb_label' => __('Enable Employer Message When New Job Submitted', 'chat-on-desk'),
        'std'      => get_option('chatondesk_new_job_customer_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'  => 'chatondesk_new_job_customer_sms',
        ...
        $data[] = array(
        'name'     => 'chatondesk_job_approve_customer_sms_status',
        'cb_label' => __('Enable Employer Message When A Job Approved', 'chat-on-desk'),
        'std'      => get_option('chatondesk_job_approve_customer_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'  => 'chatondesk_job_approve_customer_sms',
        ...
        $data[] = array(
        'name'     => 'chatondesk_edit_job_customer_sms_status',
        'cb_label' => __('Enable Employer Message When Job Edited', 'chat-on-desk'),
        'std'      => get_option('chatondesk_edit_job_customer_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'  => 'chatondesk_edit_job_customer_sms',
        ...
        $data[] = array(
        'name'     => 'chatondesk_new_application_sms_status',
        'cb_label' => __('Enable Employer Message When New Application Submitted', 'chat-on-desk'),
        'std'      => get_option('chatondesk_new_application_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );

        $data[] = array(
        'name'  => 'chatondesk_new_application_sms',
        'std'   => 'Dear [username], a candidate [candidate_name] is applied for job.',
        'label' => '',
        'desc'  => __('You can use following tokens [store_name], [job_id], [job_name] ,[username] ,[email] ,[phone] ,[candidate_name] ,[candidate_email]', 'chat-on-desk'),
        'type'  => 'textarea',
        ...
        $data[] = array(
        'name'     => 'chatondesk_new_application_candidate_sms_status',
        'cb_label' => __('Enable Candidate Message When New Application Submitted', 'chat-on-desk'),
        'std'      => get_option('chatondesk_new_application_sms_status', 1),
        'label'    => '',
        'type'     => 'checkbox',
        );
        ...
        $data[] = array(
        'name'  => 'chatondesk_new_application_candidate_sms',
...
www.chatondesk.com',
    ...
    public function sendNewJobNotification( $job_id )
    {
        ...
        $phone                       = get_user_meta($user_info->ID, 'billing_phone', true);
        $msg_enable                  = get_option('chatondesk_sms_notification');
        $new_job_customer_msg_enable = get_option('chatondesk_new_job_customer_sms_status');
        $new_job_admin_msg_enable    = get_option('chatondesk_new_job_sms_status');

        if ($msg_enable ) {
            $visitor_message = get_option('chatondesk_new_job_customer_sms');
            $admin_message   = get_option('chatondesk_new_job_sms');
    ...
    public function sendUpdatedJobNotification( $job_id )
    {
        ...
        $msg_enable                   = get_option('chatondesk_sms_notification');
        $edit_job_customer_msg_enable = get_option('chatondesk_edit_job_customer_sms_status');
        $edit_job_admin_msg_enable    = get_option('chatondesk_edit_job_sms_status');

        if ($msg_enable ) {
            $visitor_message = get_option('chatondesk_edit_job_customer_sms');
            $admin_message   = get_option('chatondesk_edit_job_sms');
