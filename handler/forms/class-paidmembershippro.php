 ...
 */
class SA_Paidmembershipspro extends \ChatOnDesk\FormInterface
{
    /**
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::PAID_MEMBERSHIP_PRO;
    ...
    public function handleForm()
    {
        ...
        add_action('chatondesk_followup_sms',  array($this, 'sendReminderSms'));
    }
    ...
    public static function add_default_setting($defaults = array())
    {
        ...
        foreach ($memberStatuses as $ks => $vs) {
            $defaults['chatondesk_pmp_general']['customer_pmp_notify_' . $vs]   = 'off';
            $defaults['chatondesk_pmp_message']['customer_sms_pmp_body_' . $vs] = '';
            $defaults['chatondesk_pmp_general']['admin_pmp_notify_' . $vs]      = 'off';
            $defaults['chatondesk_pmp_message']['admin_sms_pmp_body_' . $vs]    = '';
        }
        $defaults['chatondesk_pmp_general']['otp_enable']                       = 'off';
        $defaults['chatondesk_pmp_general']['customer_notify']                  = 'off';
        $defaults['chatondesk_pmp_renewal']['customer_notify']                  = 'off';
        $defaults['chatondesk_pmp_renewal_scheduler']['cron'][0]['frequency']   = '1';
        $defaults['chatondesk_pmp_renewal_scheduler']['cron'][0]['message']     = '';
        $defaults['chatondesk_pmp_renewal_scheduler']['cron'][1]['frequency']   = '2';
        $defaults['chatondesk_pmp_renewal_scheduler']['cron'][1]['message']     = '';
        return $defaults;
    }
    ...
    {  
        ...
        if (!$pmpro_requirebilling) {
              echo '<label for="billing_phone">'. esc_html__('Phone', 'chat-on-desk').'</label>
    ...
    {
        if (chatondesk_get_option('otp_enable', 'chatondesk_pmp_general') === 'on') {
            echo do_shortcode('[cod_verify phone_selector=" #bphone" submit_selector= " #pmpro_btn-submit"]');
        }
    ...
    public function pmproFormLoginOtp()
    {
        ...
        if ('on' === $default_login_otp && 'on' === $enabled_login_popup ) {
            echo do_shortcode('[cod_verify user_selector="#user_login" pwd_selector="#user_pass" submit_selector="#wp-submit"]');
        }  
    }
    ...
    { 
        ...
        $customerNotify    = chatondesk_get_option('customer_notify', 'chatondesk_pmp_renewal', 'on'); 
        ...
        if ('success' === $status && 'on' === $customerNotify && $expiry) {
            $scheduler_data = get_option('chatondesk_pmp_renewal_scheduler');
    ...
    {    
        ...
        $source               = 'paid-memberships-pro';
        $table_name           = $wpdb->prefix . 'chatondesk_renewal_reminders'; 
        $schedulerData        = get_option('chatondesk_pmp_reminder_scheduler');
        $today                = new \DateTime();
        ...
        if ('on' === $customerNotify && $subscription_details) { 
            foreach ( $subscription_details as $subscription ) {
                ...
                do_action('cod_send_sms', $buyerMob, $customer_msg);
            }
        }
    }
    ...
    {
        $currentVal     = chatondesk_get_option('customer_notify', 'chatondesk_pmp_renewal', 'on');
    
        $checkboxNameId = 'chatondesk_pmp_renewal[customer_notify]';
        $schedulerData  = get_option('chatondesk_pmp_renewal_scheduler');
        $templates      = array();
        $count          = 0;
        if (empty($schedulerData) === true) {
            $schedulerData['cron'][] = array(
                'frequency' => '1',
                'message'   => sprintf(__('Hello %1$s, your membership %2$s with %3$s is expired on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[userName]', '#[order_id]', '[store_name]', '[membership_expiration_date]', PHP_EOL, PHP_EOL),
            );
            $schedulerData['cron'][] = array(
                'frequency' => '2',
                'message'   => sprintf(__('Hello %1$s, your membership %2$s with %3$s is expired on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[userName]', '#[order_id]', '[store_name]', '[membership_expiration_date]', PHP_EOL, PHP_EOL),
        ...
        foreach ($schedulerData['cron'] as $key => $data) {
            ...
            $textAreaNameId  = 'chatondesk_pmp_renewal_scheduler[cron][' . $count . '][message]';
            $selectNameId    = 'chatondesk_pmp_renewal_scheduler[cron][' . $count . '][frequency]';
    ...
    public static function getCustomerTemplates()
    {
        ...
        foreach ($memberStatuses as $wws => $ms) {
            $currentVal      = chatondesk_get_option('customer_pmp_notify_' . strtolower($ms), 'chatondesk_pmp_general', 'on');
            $checkboxNameId  = 'chatondesk_pmp_general[customer_pmp_notify_' . strtolower($ms) . ']';
            $textareaNameId  = 'chatondesk_pmp_message[customer_sms_pmp_body_' . strtolower($ms) . ']';
            $defaultTemplate = chatondesk_get_option('customer_sms_pmp_body_' . strtolower($ms), 'chatondesk_pmp_message', sprintf(__('Hello %1$s, status of your membership #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[userName]', '[membership_id]', '[store_name]', $ms, PHP_EOL, PHP_EOL));
            $textBody       = chatondesk_get_option('customer_sms_pmp_body_' . strtolower($ms), 'chatondesk_pmp_message', $defaultTemplate);
        ...
        foreach ($orderStatuses as $ks => $vs) {
            if (!empty($vs)) {
                $currentVal      = chatondesk_get_option('customer_pmp_notify_' . strtolower($vs), 'chatondesk_pmp_general', 'on');
                $checkboxNameId  = 'chatondesk_pmp_general[customer_pmp_notify_' . strtolower($vs) . ']';
                $textareaNameId  = 'chatondesk_pmp_message[customer_sms_pmp_body_' . strtolower($vs) . ']';
                $defaultTemplate = chatondesk_get_option('customer_sms_pmp_body_' . strtolower($vs), 'chatondesk_pmp_message', sprintf(__('Hello %1$s, status of your order #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[order_id]', '[store_name]', $vs, PHP_EOL, PHP_EOL));
                $textBody       = chatondesk_get_option('customer_sms_pmp_body_' . strtolower($vs), 'chatondesk_pmp_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        $ws = 0;
        foreach ($memberStatuses as $wws => $ms) {
                $currentVal      = chatondesk_get_option('admin_pmp_notify_' . strtolower($ms), 'chatondesk_pmp_general', 'on');
                $checkboxNameId  = 'chatondesk_pmp_general[admin_pmp_notify_' . strtolower($ms) . ']';
                $textareaNameId  = 'chatondesk_pmp_message[admin_sms_pmp_body_' . strtolower($ms) . ']';
                $defaultTemplate = chatondesk_get_option('admin_sms_pmp_body_' . strtolower($ms), 'chatondesk_pmp_message', sprintf(__('Hello admin, status of your membership with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $ms, PHP_EOL, PHP_EOL));
                $textBody = chatondesk_get_option('admin_sms_pmp_body_' . strtolower($ms), 'chatondesk_pmp_message', $defaultTemplate);
        ...
        foreach ($orderStatuses as $ks => $vs) {
            ...
            if (!empty($vs)) {
                $currentVal      = chatondesk_get_option('admin_pmp_notify_' . strtolower($vs), 'chatondesk_pmp_general', 'on');
                $checkboxNameId  = 'chatondesk_pmp_general[admin_pmp_notify_' . strtolower($vs) . ']';
                $textareaNameId  = 'chatondesk_pmp_message[admin_sms_pmp_body_' . strtolower($vs) . ']';
                $defaultTemplate = chatondesk_get_option('admin_sms_pmp_body_' . strtolower($vs), 'chatondesk_pmp_message', sprintf(__('Hello admin, status of your membership with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $vs, PHP_EOL, PHP_EOL));
                $textBody = chatondesk_get_option('admin_sms_pmp_body_' . strtolower($vs), 'chatondesk_pmp_message', $defaultTemplate);
    ...
    public function sendSmsMembershipActive($level_id, $user_id, $cancel_level)
    {
        if (!empty($level_id)) {
            ...
            $customerMessage     = chatondesk_get_option('customer_sms_pmp_body_active', 'chatondesk_pmp_message', '');
            $customerNotify    = chatondesk_get_option('customer_pmp_notify_active', 'chatondesk_pmp_general', 'on');
    ...
    public function sendSmsMembershipCancel($level_id, $user_id, $old_levels, $cancel_level)
    {
        if (!empty($cancel_level)) {
            ...
            $customerMessage     = chatondesk_get_option('customer_sms_pmp_body_cancel', 'chatondesk_pmp_message', '');
            $customerNotify    = chatondesk_get_option('customer_pmp_notify_cancel', 'chatondesk_pmp_general', 'on');
            if (($customerNotify === 'on' && $customerMessage !== '')) {
                  $buyerMessage = $this->parseSmsBody($data,  $customerMessage);
                do_action('cod_send_sms', $buyerNumber, $buyerMessage);
            }
    ...
    public function sendSmsOn($order)
    {
        if (!empty($order->billing)) {
            $status          = $order->status;
            $customerMessage        = chatondesk_get_option('customer_sms_pmp_body_' . $status, 'chatondesk_pmp_message', '');
            $customerNotify         = chatondesk_get_option('customer_pmp_notify_' . $status, 'chatondesk_pmp_general', 'on');
            $user_id                = $order->user_id;
            $buyerNumber            = get_user_meta($user_id, 'pmpro_bphone', true);
            if (($customerNotify === 'on' && $customerMessage !== '')) {
            ...
            if (empty($adminPhoneNumber) === false) {
                $adminNotify        = chatondesk_get_option('admin_pmp_notify_' . $status, 'chatondesk_pmp_general', 'on');
                $adminMessage       = chatondesk_get_option('admin_sms_pmp_body_' . $status, 'chatondesk_pmp_message', '');
