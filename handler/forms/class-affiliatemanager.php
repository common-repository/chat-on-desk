 ...
 */
class AffiliateManager extends \ChatOnDesk\FormInterface
{
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::AFFILIATE_MANAGER_REG;
     ...
     * @var stirng
     */
    private $form_phone_ver = \ChatOnDesk\FormSessionVars::AFFILIATE_MANAGER_PHONE_VER;
    ...
    public function addDefaultSetting( $defaults = array() )
    {
        ...
        foreach ( $wpam_statuses as $ks => $vs ) {
            $defaults['chatondesk_wpam_general'][ 'wpam_admin_notification_' . $vs ] = 'off';
            $defaults['chatondesk_wpam_general'][ 'wpam_order_status_' . $vs ]       = 'off';
            $defaults['chatondesk_wpam_message'][ 'wpam_admin_sms_body_' . $vs ]     = '';
            $defaults['chatondesk_wpam_message'][ 'wpam_sms_body_' . $vs ]           = '';
        }
        return $defaults;
    }
    ...
    public static function afterAddedWpamTransaction( $data = array() )
    {
        ...
        $buyer_sms_notify = chatondesk_get_option(
            'wpam_order_status_' . $status, 
            'chatondesk_wpam_general', 'on'
        );
        $admin_sms_notify = chatondesk_get_option(
            'wpam_admin_notification_' . $status, 
            'chatondesk_wpam_general', 'on'
        );

        $buyer_sms_content = chatondesk_get_option(
            'wpam_sms_body_' . $status, 
            'chatondesk_wpam_message',
            SmsAlertMessages::showMessage('DEFAULT_WPAM_BUYER_SMS_TRANS_STATUS_CHANGED')
        );

        $admin_sms_content = chatondesk_get_option(
        ...
        if (count($am_user) > 0 ) {
            ...
            if ('on' === $buyer_sms_notify && ! empty($buyer_sms_content) ) {
                ...
                $response              = Chatondesk::sendsms($wpam_user);
            }
            ...
            if ('on' === $admin_sms_notify && ! empty($admin_phone_number) && ! empty($admin_sms_content) ) {
                ...
                $response               = Chatondesk::sendsms($wpam_admin);
            }
        }
    }
    ...
    public static function afterChangedWpamStatus( $affiliate_id )
    {
        ...
        if (count($am_user) > 0 ) {
            ...
            $admin_phone_number = chatondesk_get_option(
                'sms_admin_phone',
                'chatondesk_message', ''
            );
            if ('on' === $admin_sms_notify && ! empty($admin_phone_number) && ! empty($admin_sms_content) ) {
    ...
    public function processPhoneAndStartOTPVerificationProcess( $data )
    {
        ...
        chatondesk_site_challenge_otp($username, $email, $errors, $data['_phoneNumber'], 'phone', null, $data, false);
    }
    ...
    public static function getCustomerTemplates()
    {
        ...
        foreach ( $wpam_statuses as $ks  => $vs ) {
            ...
            $current_val = chatondesk_get_option('wpam_order_status_' . $vs, 'chatondesk_wpam_general', 'on');

            $checkbox_name_id = 'chatondesk_wpam_general[wpam_order_status_' . $vs . ']';
            $textarea_name_id = 'chatondesk_wpam_message[wpam_sms_body_' . $vs . ']';

            $text_body = chatondesk_get_option('wpam_sms_body_' . $vs, 'chatondesk_wpam_message', SmsAlertMessages::showMessage('DEFAULT_WPAM_BUYER_SMS_STATUS_CHANGED'));
        ...
        foreach ( $wpam_transaction as $ks  => $vs ) {
            ...
            $text_body = chatondesk_get_option('wpam_sms_body_' . $vs, 'chatondesk_wpam_message', SmsAlertMessages::showMessage('DEFAULT_WPAM_BUYER_SMS_TRANS_STATUS_CHANGED'));
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ( $wpam_statuses as $ks  => $vs ) {
            ...
            $current_val = chatondesk_get_option('wpam_admin_notification_' . $vs, 'chatondesk_wpam_general', 'on');

            $checkbox_name_id = 'chatondesk_wpam_general[wpam_admin_notification_' . $vs . ']';
            $textarea_name_id = 'chatondesk_wpam_message[wpam_admin_sms_body_' . $vs . ']';

            $text_body = chatondesk_get_option('wpam_admin_sms_body_' . $vs, 'chatondesk_wpam_message', SmsAlertMessages::showMessage('DEFAULT_WPAM_ADMIN_SMS_STATUS_CHANGED'));
        ...
        foreach ( $wpam_transaction as $ks  => $vs ) {
            ...
            $text_body = chatondesk_get_option(
                'wpam_admin_sms_body_' . $vs, 'chatondesk_wpam_message', 
                SmsAlertMessages::showMessage('DEFAULT_WPAM_ADMIN_SMS_TRANS_STATUS_CHANGED')
            );
