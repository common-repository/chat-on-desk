 ...
 */
class WpLoyalty extends \ChatOnDesk\FormInterface
{
    /**
     * Construct function.
     *
    ...
    public static function add_default_setting($defaults = array())
    {
        ...
        foreach ($bookingStatuses as $ks) {
            $defaults['chatondesk_wpl_general']['customer_wpl_notify_' . $ks]   = 'off';
            $defaults['chatondesk_wpl_message']['customer_sms_wpl_body_' . $ks] = '';
        }
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates           = [];
        $currentVal      = chatondesk_get_option('customer_wpl_notify_points', 'chatondesk_wpl_general', 'on');

        $checkboxNameId  = 'chatondesk_wpl_general[customer_wpl_notify_points]';
        $textareaNameId  = 'chatondesk_wpl_message[customer_sms_wpl_body_points]';

        $defaultTemplate = chatondesk_get_option('customer_sms_wpl_body_points', 'chatondesk_wpl_message', sprintf(__('%1$s: %2$s store credits added in your account! Total store credits are %3$s. %4$sPowered by%5$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', '[points]', '[total_points]', PHP_EOL, PHP_EOL));


        $textBody       = chatondesk_get_option('customer_sms_wpl_body_points', 'chatondesk_wpl_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('customer_wpl_notify_rewards', 'chatondesk_wpl_general', 'on');

        $checkboxNameId  = 'chatondesk_wpl_general[customer_wpl_notify_rewards]';
        $textareaNameId  = 'chatondesk_wpl_message[customer_sms_wpl_body_rewards]';

        $defaultTemplate = chatondesk_get_option('customer_sms_wpl_body_rewards', 'chatondesk_wpl_message', sprintf(__('Dear %1$s, %2$s loyalty points has been debited from your account.Your total points : %3$s. %4$sPowered by%5$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[points]', '[total_points]', PHP_EOL, PHP_EOL));


        $textBody       = chatondesk_get_option('customer_sms_wpl_body_rewards', 'chatondesk_wpl_message', $defaultTemplate);
    ...
    {    
        $customerMessage   = chatondesk_get_option('customer_sms_wpl_body_points', 'chatondesk_wpl_message', '');
        
        $customerNotify    = chatondesk_get_option('customer_wpl_notify_points', 'chatondesk_wpl_general', 'on');
    ...
    public function sendSmsOnReward($insert_id,$data)
    {
        if ($data['action_type'] =='redeem_point' 
            ...
            $buyerNumber = $data['billing_phone'];
            $customerMessage   = chatondesk_get_option('customer_sms_wpl_body_rewards', 'chatondesk_wpl_message', '');
            $customerNotify    = chatondesk_get_option('customer_wpl_notify_rewards', 'chatondesk_wpl_general', 'on');
