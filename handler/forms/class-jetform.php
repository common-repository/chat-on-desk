 ...
 * JetForm class.
 */
class JetForm extends \ChatOnDesk\FormInterface
{
    ...
    public function jetformSubmissionComplete($form_data, $entry_id )
    {
        ...
        $form_enable      = chatondesk_get_option(
            'jetform_order_status_' . $form_id,
            'chatondesk_jetform_general', 'on'
        );
        
        $phone_field      = chatondesk_get_option(
            'jetform_sms_phone_' . $form_id, 
            'chatondesk_jetform_general', ''
        );
        ...
        if ('on' === $form_enable && 'on' === $buyer_sms_notify ) {
            ...
            do_action('cod_send_sms', $mobile, $msg);
        }
        if ('on' === $admin_sms_notify ) {
            $admin_phone_number = chatondesk_get_option(
                'sms_admin_phone', 'chatondesk_message', ''
            );
            ...
            if (! empty($admin_phone_number) ) {
                ...
                do_action(
                    'cod_send_sms', $admin_phone_number,
