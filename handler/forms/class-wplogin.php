 ...
 * WPLogin class.
 **/
class WPLogin extends \ChatOnDesk\FormInterface
{
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::WP_LOGIN_REG_PHONE;
     ...
     * @var stirng
     */
    private $form_session_var2 = \ChatOnDesk\FormSessionVars::WP_DEFAULT_LOGIN;
     ...
     * @var stirng
     */
    private $form_session_var3 = \ChatOnDesk\FormSessionVars::WP_LOGIN_WITH_OTP;
    ...
    {
		$forms['cod_lwo_form'] = array( "form_name" => "Chat On Desk Login With OTP" );
		$forms['cod_swm_form'] = array( "form_name" => "Chat On Desk Signup With Mobile" );
        return $forms;
	}
    ...
    public function routeData()
    {
        ...
        case 'chatondesk_ajax_form_validate':
            ...
            break;
        case 'chatondesk_ajax_login_with_otp':
            $this->handleLoginWithOtp();
            break;
        case 'chatondesk_ajax_login_popup':
            $this->handleLoginPopup();
            break;
        case 'chatondesk_verify_login_with_otp':
    ...
    public function handleLoginPopup()
    {
        ...
        chatondesk_site_challenge_otp($username, null, null, $phone_number, 'phone', $password, \ChatOnDesk\SmsAlertUtility::currentPageUrl(), true);
    }
    ...
    {
		$verify = check_ajax_referer('chatondesk_wp_loginwithotp_nonce', 'chatondesk_loginwithotp_nonce', false);
        if (!$verify) {
            wp_send_json(\ChatOnDesk\SmsAlertUtility::_create_json_response(__('Sorry, nonce did not verify.', 'chat-on-desk'), 'error'));
        }
		if (is_plugin_active('google-captcha/google-captcha.php')) {
			$check_result = apply_filters( 'gglcptch_verify_recaptcha', true, 'string', 'cod_lwo_form' );
        ...
        if (isset($_REQUEST['username']) ) {
            ...
            $billing_phone = Chatondesk::checkPhoneNos($phone_number);
			if (\ChatOnDesk\SmsAlertUtility::isBlank($phone_number)) {
                ...
                $message = str_replace('##phone##', $phone_number, $phoneCodLogic->_get_otp_invalid_format_message());
            ...
            if (! empty($user_login) ) {
                ...
                wp_send_json(\ChatOnDesk\SmsAlertUtility::_create_json_response(SmsAlertMessages::showMessage('PHONE_NOT_FOUND'), 'error'));
            }
        }
    }
    ...
    public function smsalertDisplayLoginWithOtp($form = null, $args=array())
    {
        if ($form == null || is_array($form) || $form == 'login') {
            echo '<div class="lwo-container"><div class="cod_or">OR</div><button type="button" class="button cod_myaccount_btn" name="cod_myaccount_btn_login" value="' . __('Login with OTP', 'chat-on-desk') . '" style="width: 100%;box-sizing: border-box;display:block">' . __('Login with OTP', 'chat-on-desk') . '</button></div>';
            if (is_plugin_active('google-captcha/google-captcha.php')) {
    ...
    public function xooElAddLoginOtpPopup()
    {
		...
		{
			if(!jQuery(this).hasClass("cod-login-form"))
			{
			jQuery(this).addClass("'.$unique_class.' cod-login-form");
			}		
		});		
		</script>';    
            echo do_shortcode('[cod_verify user_selector="xoo-el-username" pwd_selector="xoo-el-password" submit_selector=".'.$unique_class.' .xoo-el-login-btn"]');
        }
    }
    ...
    public function addLoginOtpPopup()
    {
		...
		jQuery("form.login").each(function () 
		{
            ...
            echo do_shortcode('[cod_verify user_selector="#username" pwd_selector="#password" submit_selector=".'.$unique_class.'.login :submit"]');
        }
    }
    ...
    public static function isFormEnabled()
    {
        ...
        return ( $islogged && ( chatondesk_get_option('buyer_login_otp', 'chatondesk_general') === 'on' || chatondesk_get_option('login_with_otp', 'chatondesk_general') === 'on' ) ) ? true : false;
    }
    ...
    public function byPassLogin( $user_role )
    {
        ...
        $excluded_roles = chatondesk_get_option('admin_bypass_otp_login', 'chatondesk_general', array());
    ...
    {
        return ( chatondesk_get_option('allow_multiple_user', 'chatondesk_general') === 'on' ) ? true : false;
    }
    ...
    public function handleWpLoginCreateUserAction( $postdata )
    {
        ...
        update_user_meta($user->data->ID, $this->phone_number_key, sanitize_text_field($postdata['cod_phone_number']));
    ...
    public function loginWpUser( $user_log, $extra_data = null )
    {
        ...
        $redirect = \ChatOnDesk\SmsAlertUtility::isBlank($extra_data) ? site_url() : $extra_data;
    ...
    {
        \ChatOnDesk\SmsAlertUtility::checkSession();
        $login_with_otp_enabled = ( chatondesk_get_option('login_with_otp', 'chatondesk_general') === 'on' ) ? true : false;
        $password='';
        if (empty($password) ) {
    ...
    public function handleSmsalertWpLogin( $user, $username, $password )
    {
        ...
        if (( is_array($_SESSION) && array_key_exists($this->form_session_var, $_SESSION) && strcasecmp($_SESSION[ $this->form_session_var ], 'validated') === 0 ) && ! empty($_POST['cod_phone_number']) ) {
            update_user_meta($user->data->ID, $this->phone_number_key, sanitize_text_field(wp_unslash($_POST['cod_phone_number'])));
            $this->unsetOTPSessionVariables();
        }
        ...
        if (isset($_SESSION['cod_login_mobile_verified']) ) {
            unset($_SESSION['cod_login_mobile_verified']);
            return $user;
        }
        ...
        if (( chatondesk_get_option('buyer_login_otp', 'chatondesk_general') === 'off' && chatondesk_get_option('login_with_otp', 'chatondesk_general') === 'on' ) ) {
            return $user;
        }
    ...
    {
        if (! $this->checkWpLoginByPhoneNumber() || ! \ChatOnDesk\SmsAlertUtility::validatePhoneNumber($username) ) {
            return $user;
        }
    ...
    public static function getUserFromPhoneNumber( $username, $key )
    {
        ...
        $wcc_ph     = Chatondesk::checkPhoneNos($username);
        $wocc_ph    = Chatondesk::checkPhoneNos($username, false);
        $wth_pls_ph = '+' . $wcc_ph;
    ...
    public function askPhoneAndStartVerification( $user, $key, $username, $phone_number )
    {
        ...
        if (! $this->checkWpLoginRegisterPhone() ) {
            chatondesk_site_otp_validation_form(null, null, null, SmsAlertMessages::showMessage('PHONE_NOT_FOUND'), null, null);
        } else {
            \ChatOnDesk\SmsAlertUtility::initialize_transaction($this->form_session_var);
            chatondesk_external_phone_validation_form(\ChatOnDesk\SmsAlertUtility::currentPageUrl(), $user->data->user_login, __('A new security system has been enabled for you. Please register your phone to continue.', 'chat-on-desk'), $key, array( 'user_login' => $username ));
        }
    }
    ...
    public function fetchPhoneAndStartVerification( $user, $key, $username, $password, $phone_number )
    {
        ...
        chatondesk_site_challenge_otp($username, null, null, $phone_number, 'phone', $password, \ChatOnDesk\SmsAlertUtility::currentPageUrl(), false);
    }
    ...
    public function handleWpLoginAjaxSendOtp( $data )
    {
        ...
        if (! $this->checkWpLoginRestrictDuplicates()
            && ! \ChatOnDesk\SmsAlertUtility::isBlank($this->getUserFromPhoneNumber($data['billing_phone'], $this->phone_number_key)) 
        ) {
            wp_send_json(\ChatOnDesk\SmsAlertUtility::_create_json_response(__('Phone Number is already in use. Please use another number.', 'chat-on-desk'), \ChatOnDesk\SmsAlertConstants::ERROR_JSON_TYPE));
        } elseif (isset($_SESSION[ $this->form_session_var ]) ) {
            chatondesk_site_challenge_otp('ajax_phone', '', null, trim($data['billing_phone']), 'phone', null, $data, null);
        }
    }
    ...
    public function handle_failed_verification( $user_login, $user_email, $phone_number )
    {
        ...
        if (isset($_SESSION[ $this->form_session_var2 ]) ) {
            chatondesk_site_otp_validation_form($user_login, $user_email, $phone_number, SmsAlertMessages::showMessage('INVALID_OTP'), 'phone', false);
        }
    ...
    public function handle_post_verification( $redirect_to, $user_login, $user_email, $password, $phone_number, $extra_data )
    {
        ...
        if (isset($_SESSION[ $this->form_session_var ]) ) {
            $_SESSION['cod_login_mobile_verified'] = true;
