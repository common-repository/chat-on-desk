 ...
 * jetPackCRM class
 */
class jetPackCRM extends \ChatOnDesk\FormInterface
{
    ...
    public static function add_default_setting($defaults=[])
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $defaults['chatondesk_jcm_general']['customer_jcm_notify_'.$vs]   = 'off';
            $defaults['chatondesk_jcm_message']['customer_sms_jcm_body_'.$vs] = '';
            $defaults['chatondesk_jcm_general']['admin_jcm_notify_'.$vs]      = 'off';
            $defaults['chatondesk_jcm_message']['admin_sms_jcm_body_'.$vs]    = '';
        }
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates = [];
        foreach ($bookingStatuses as $ks  => $vs) {
            $currentVal = chatondesk_get_option('customer_jcm_notify_'.strtolower($vs), 'chatondesk_jcm_general', 'on');

            $checkboxNameId = 'chatondesk_jcm_general[customer_jcm_notify_'.strtolower($vs).']';
            $textareaNameId = 'chatondesk_jcm_message[customer_sms_jcm_body_'.strtolower($vs).']';

            $defaultTemplate = chatondesk_get_option('admin_sms_jcm_body_'.strtolower($vs), 'chatondesk_jcm_message', sprintf(__('Hello %1$s, status of your contact #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[name]', '[contact_id]', '[store_name]', $vs, PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_jcm_body_'.strtolower($vs), 'chatondesk_jcm_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        $templates = [];
        foreach ($bookingStatuses as $ks  => $vs) {
            $currentVal     = chatondesk_get_option('admin_jcm_notify_'.strtolower($vs), 'chatondesk_jcm_general', 'on');
            $checkboxNameId = 'chatondesk_jcm_general[admin_jcm_notify_'.strtolower($vs).']';
            $textareaNameId = 'chatondesk_jcm_message[admin_sms_jcm_body_'.strtolower($vs).']';

            $defaultTemplate = chatondesk_get_option('admin_sms_jcm_body_'.strtolower($vs), 'chatondesk_jcm_message', sprintf(__('Hello admin, status of your contact with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $vs, PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('admin_sms_jcm_body_'.strtolower($vs), 'chatondesk_jcm_message', $defaultTemplate);
    ...
    public function sendsmsNewContact($cID)
    {
        ...
        $buyerSmsData     = [];
        $customerMessage  = chatondesk_get_option('customer_sms_jcm_body_new', 'chatondesk_jcm_message', '');
        $customerRrNotify = chatondesk_get_option('customer_jcm_notify_new', 'chatondesk_jcm_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminRrNotify = chatondesk_get_option('admin_jcm_notify_new', 'chatondesk_jcm_general', 'on');
            $adminMessage  = chatondesk_get_option('admin_sms_jcm_body_new', 'chatondesk_jcm_message', '');
    ...
    public function sendsmsContactUpdate($cID, $data)
    {
        ...
        $customerMessage = chatondesk_get_option('customer_sms_jcm_body_'.$bookingStatus, 'chatondesk_jcm_message', '');
        $customerNotify  = chatondesk_get_option('customer_jcm_notify_'.$bookingStatus, 'chatondesk_jcm_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminNotify = chatondesk_get_option('admin_jcm_notify_'.$bookingStatus, 'chatondesk_jcm_general', 'on');

            $adminMessage = chatondesk_get_option('admin_sms_jcm_body_'.$bookingStatus, 'chatondesk_jcm_message', '');
