 ...
 */
class UltimateMemberRegistrationForm extends \ChatOnDesk\FormInterface
{
     ...
     * @returnstirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::UM_DEFAULT_REG;
     ...
     * @returnstirng
     */
    private $form_session_var3 = \ChatOnDesk\FormSessionVars::UMR_POPUP;
    ...
    public function handleForm()
    {
        ...
        if (chatondesk_get_option('reset_password', 'chatondesk_general') === 'on' ) {
        ...
        if (! empty($_REQUEST['option']) && sanitize_text_field(wp_unslash($_REQUEST['option'])) === 'chatondesk-um-reset-pwd-action' ) {
            $this->handleSmsalertChangedPwd($_POST);
            wp_enqueue_style('wpv_cod_common_style', COD_MOV_CSS_URL, array(), \ChatOnDesk\SmsAlertConstants::SA_VERSION, false);
        }

        if (! empty($_REQUEST['cod_um_reset_pwd']) ) {
    ...
    public static function add_default_setting( $defaults = array() )
    {
        ...
        foreach ($ultimate_actions as $status=>$label) {
            $defaults['chatondesk_um_general'][$status] = 'off';
            $defaults['chatondesk_um_message'][$status] = '';
        }
        return $defaults;
    }
    ...
    public static function getUltimatememberTemplates()
    {
        ...
        foreach ($ultimate_actions as $status=>$label) {
            $current_val      = chatondesk_get_option($status, 'chatondesk_um_general', 'on');
            $checkbox_name_id = 'chatondesk_um_general['.$status.']';
            $textarea_name_id = 'chatondesk_um_message['.$status.']';
            $text_body        = chatondesk_get_option($status, 'chatondesk_um_message', sprintf(__('Dear %1$s, your account status with %2$s has been changed to %3$s.%4$sPowered by%5$swww.chatondesk.com', 'chat-on-desk'), '[username]', '[store_name]', '[status]', PHP_EOL, PHP_EOL));
    ...
    public function sendSmsStatusChanged( $status, $user_id )
    {
        ...
        $chatondesk_um_enabled_msg  = chatondesk_get_option($status, 'chatondesk_um_general', 'on');
        $chatondesk_um_msg = chatondesk_get_option($status, 'chatondesk_um_message', '');
        if ('on' === $chatondesk_um_enabled_msg && '' !== $chatondesk_um_msg ) {
            $search = array(
            '[username]',
            '[email]',
            '[billing_phone]',
            ...
            $sms_body = str_replace($search, $replace, $chatondesk_um_msg);
            do_action('cod_send_sms', $phone, $sms_body);
        }
    }
    ...
    public function addCountryCode( $args )
    {
        ...
        if ('register' === $args['mode'] && 'on' === $enabled_country ) {
			...
			jQuery(document).ready(function(){				
				initialiseCodCountrySelector("#billing_phone-'.$args['form_id'].'");
			})
			</script>';            
        }
        echo '<script>			
    ...
    public function handleSmsalertChangedPwd( $post_data )
    {
        ...
        $error            = '';
        $new_password     = ! empty($post_data['chatondesk_user_newpwd']) ? $post_data['chatondesk_user_newpwd'] : '';
        $confirm_password = ! empty($post_data['chatondesk_user_cnfpwd']) ? $post_data['chatondesk_user_cnfpwd'] : '';
        ...
        if (! empty($error) ) {
            chatondeskAskForResetPassword($_SESSION['user_login'], $_SESSION['phone_number_mo'], $error, 'phone', false);
        }

        $user = get_user_by('login', $_SESSION['user_login']);
        reset_password($user, $new_password);
        $this->unsetOTPSessionVariables();
        exit(wp_redirect(esc_url(add_query_arg('cod_um_reset_pwd', true, um_get_core_page('password-reset')))));
    }
    ...
    public function umFormAddShortcode( $args )
    {
        ...
        if ('on' === $default_login_otp && 'on' === $enabled_login_popup ) {
            if ('login' === $args['mode'] ) {
                echo do_shortcode('[cod_verify user_selector="#username-' . esc_attr($args['form_id']) . '" pwd_selector="#user_password-' . esc_attr($args['form_id']) . '" submit_selector=".um-login #um-submit-btn"]');
            }
        }
        if ('on' === $buyer_signup_otp && 'on' === $enabled_register_popup ) {
            if ('register' === $args['mode'] ) {
                echo do_shortcode('[cod_verify phone_selector="#billing_phone-' . esc_attr($args['form_id']) . '" submit_selector=".um-register #um-submit-btn"]');
            }
        }
    }
    ...
    {
        echo '<p class="um-notice success"><i class="um-icon-ios-close-empty" onclick="jQuery(this).parent().fadeOut();"></i>' . __('Password Changed Successfully.', 'chat-on-desk') . '</p>';
    }
    ...
    {
        \ChatOnDesk\SmsAlertUtility::checkSession();
        if (isset($_SESSION['cod_um_mobile_verified']) ) {
            unset($_SESSION['cod_um_mobile_verified']);
            return false;
        }
        ...
        if (isset($user_phone) && \ChatOnDesk\SmsAlertUtility::isBlank($user_phone) ) {
            UM()->form()->add_error('registration-error-invalid-phone', __('Please enter phone number.', 'chat-on-desk'));
        }
        
        global $phoneCodLogic;
        if (! \ChatOnDesk\SmsAlertUtility::validatePhoneNumber($user_phone) ) {
            UM()->form()->add_error('billing_phone_error', str_replace('##phone##', $user_phone, $phoneCodLogic->_get_otp_invalid_format_message()));
        }
        
        if (chatondesk_get_option('allow_multiple_user', 'chatondesk_general') !== 'on' && ! \ChatOnDesk\SmsAlertUtility::isBlank($args['billing_phone']) ) {
            $getusers = \ChatOnDesk\SmsAlertUtility::getUsersByPhone('billing_phone', $args['billing_phone']);
            if (count($getusers) > 0 ) {
                UM()->form()->add_error('billing_phone', __('An account is already registered with this mobile number. Please login.', 'chat-on-desk'));
            }
        }
    ...
    {
        chatondesk_site_challenge_otp($username, $email, $errors, $phone_no, 'phone', $password);
    }
    ...
    public static function isFormEnabled()
    {
        ...
        return ( 'on' === chatondesk_get_option('buyer_signup_otp', 'chatondesk_general') && $islogged ) ? true : false;
    }
    ...
    {
        chatondesk_site_challenge_otp($username, $email, $errors, $phone_number, 'phone', $password, $extra_data);
    }
    ...
    public function handle_post_verification( $redirect_to, $user_login, $user_email, $password, $phone_number, $extra_data )
    {
        ...
        $_SESSION['cod_um_mobile_verified'] = true;
        ...
        if (isset($_SESSION[ $this->form_session_var2 ]) ) {
            chatondeskAskForResetPassword($_SESSION['user_login'], $_SESSION['phone_number_mo'], SmsAlertMessages::showMessage('CHANGE_PWD'), 'phone', false, 'chatondesk-um-reset-pwd-action');
        } 
    }
