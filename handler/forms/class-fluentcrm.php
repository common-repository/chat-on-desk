 ...
 * fluent-crm 
 */
class Fluentcrm extends \ChatOnDesk\FormInterface
{
    /**
     * Construct function.
     *
    ...
    public static function add_default_setting($defaults = array())
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $defaults['chatondesk_ft_general']['customer_ft_notify_' . $vs]   = 'off';
            $defaults['chatondesk_ft_message']['customer_sms_ft_body_' . $vs] = '';
            $defaults['chatondesk_ft_general']['admin_ft_notify_' . $vs]      = 'off';
            $defaults['chatondesk_ft_message']['admin_sms_ft_body_' . $vs]    = '';
        }
        return $defaults;
    }
    ...
    public static function getCustomerTemplates()
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $currentVal      = chatondesk_get_option('customer_ft_notify_' . strtolower($vs), 'chatondesk_ft_general', 'on');
            $checkboxNameId  = 'chatondesk_ft_general[customer_ft_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_ft_message[customer_sms_ft_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('customer_sms_ft_body_' . strtolower($vs), 'chatondesk_ft_message', sprintf(__('Hello %1$s, status of your contact with %2$s has been changed to %3$s.%4$sPowered by%5$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[store_name]', $vs, PHP_EOL, PHP_EOL));


            $textBody = chatondesk_get_option('customer_sms_ft_body_' . strtolower($vs), 'chatondesk_ft_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $currentVal      = chatondesk_get_option('admin_ft_notify_' . strtolower($vs), 'chatondesk_ft_general', 'on');
            $checkboxNameId  = 'chatondesk_ft_general[admin_ft_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_ft_message[admin_sms_ft_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('admin_sms_ft_body_' . strtolower($vs), 'chatondesk_ft_message', sprintf(__('Hello admin, status of your contact with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $vs, PHP_EOL, PHP_EOL));


            $textBody       = chatondesk_get_option('admin_sms_ft_body_' . strtolower($vs), 'chatondesk_ft_message', $defaultTemplate);
    ...
    public function sendSmsOn($contact, $status)
    {
        ...
        $customerMessage      = chatondesk_get_option('customer_sms_ft_body_' . $status, 'chatondesk_ft_message', '');
        $customerNotify = chatondesk_get_option('customer_ft_notify_' . $status, 'chatondesk_ft_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminNotify      = chatondesk_get_option('admin_ft_notify_' . $status, 'chatondesk_ft_general', 'on');
            $adminMessage     = chatondesk_get_option('admin_sms_ft_body_' . $status, 'chatondesk_ft_message', '');
