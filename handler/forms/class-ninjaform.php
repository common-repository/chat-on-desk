 ...
 * NinjaForm class.
 */
class NinjaForm extends \ChatOnDesk\FormInterface
{
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::NF_FORMS;
     ...
     * @var stirng
     */
    private $form_phone_ver = \ChatOnDesk\FormSessionVars::NF_PHONE_VER;
    ...
    {
        echo do_shortcode('[cod_verify id="form1" phone_selector=".cod-phone-field" submit_selector= ".cod_ninja_submit_btn" ]');
    }
    ...
    public function saNinjaHandleJsScript($form_id = null)
    {
        ...
        $otp_enable               = chatondesk_get_option('ninja_otp_' . $form_id, 'chatondesk_ninja_general', 'on');
        
        $country_flag_enable    = chatondesk_get_option('checkout_show_country_code', 'chatondesk_general');
        ...
        if ('on' === $country_flag_enable ) {
			...
			function addflag() {
	          initialiseCodCountrySelector(".cod-phone-field.phone-valid");
			}
			';
        }
        wp_add_inline_script("cod-handle-footer", $ninja_js);
    }
    ...
    {
        $form_id            = $form->get_id();
        $form_enable        = chatondesk_get_option('ninja_order_status_' . $form_id, 'chatondesk_ninja_general', 'on');
        $otp_enable         = chatondesk_get_option('ninja_otp_' . $form_id, 'chatondesk_ninja_general', 'on');
        $phone_field        = chatondesk_get_option('ninja_sms_phone_' . $form_id, 'chatondesk_ninja_general', '');
        
        if ($settings['key'] === $phone_field ) {
            $settings['element_class'] = 'cod-phone-field phone-valid';
        }
        return $settings;
    }
    ...
    public function addCustomButton( $settings, $form )
    {
        ...
        if ('on' === $form_enable && 'on' === $otp_enable ) {
            $settings['element_class'] = 'cod_ninja_submit_btn cod-default-btn-hide';
            $settings['afterField'] = '
					...
					<div class="nf-field">
						<div class="field-wrap submit-wrap">
							...
							<div class="nf-field-element">
								<input id="cod_verify_'.$uniqueNo.'" class="cod-otp-btn-init ninja-forms-field nf-element chatondesk_otp_btn_submit" value="' . __('Submit', 'chat-on-desk') . '" type="button">
							</div>
						</div>
					</div>
				';
			...
			send_cod_otp(this,".cod_ninja_submit_btn",".cod-phone-field","","");
		    });
			...
			';
            wp_add_inline_script("cod-handle-footer", $js);
    ...
    {
        if (is_plugin_active('ninja-forms/ninja-forms.php') ) {
            add_filter('codDefaultSettings', __CLASS__ . '::addDefaultSetting', 1, 2);
    ...
    public static function addTabs( $tabs = array() )
    {
        ...
        'kb_link'      => array(
        'href'   => 'https://kb.smsalert.co.in/knowledgebase/how-to-integrate-chatondesk-with-ninja-forms/#',
        'target' => '_blank',
    ...
    public static function addDefaultSetting( $defaults = array() )
    {
        ...
        foreach ( $wpam_statuses as $ks => $vs ) {
            $defaults['chatondesk_ninja_general'][ 'ninja_admin_notification_' . $ks ] = 'off';
            $defaults['chatondesk_ninja_general'][ 'ninja_order_status_' . $ks ]       = 'off';
            $defaults['chatondesk_ninja_general'][ 'ninja_message_' . $ks ]            = 'off';
            $defaults['chatondesk_ninja_message'][ 'ninja_admin_sms_body_' . $ks ]     = '';
            $defaults['chatondesk_ninja_message'][ 'ninja_sms_body_' . $ks ]           = '';
            $defaults['chatondesk_ninja_general'][ 'ninja_sms_phone_' . $ks ]          = '';
            $defaults['chatondesk_ninja_general'][ 'ninja_sms_otp_' . $ks ]            = '';
            $defaults['chatondesk_ninja_general'][ 'ninja_otp_' . $ks ]                = '';
            $defaults['chatondesk_ninja_message'][ 'ninja_otp_sms_' . $ks ]            = '';
        }
        return $defaults;
    }
    ...
    public static function smsalertSendSmsFormSubmit( $form_data )
    {
        ...
        if (! empty($form_data) ) {
            $billing_phone = '';
            $phone_field   = chatondesk_get_option('ninja_sms_phone_' . $form_data['form_id'], 'chatondesk_ninja_general', '');
            ...
            $form_enable      = chatondesk_get_option('ninja_message_' . $form_data['form_id'], 'chatondesk_ninja_general', 'on');
            $buyer_sms_notify = chatondesk_get_option('ninja_order_status_' . $form_data['form_id'], 'chatondesk_ninja_general', 'on');
            $admin_sms_notify = chatondesk_get_option('ninja_admin_notification_' . $form_data['form_id'], 'chatondesk_ninja_general', 'on');

            if ('on' === $form_enable && 'on' === $buyer_sms_notify ) {
                if (! empty($billing_phone) ) {
                    $buyer_sms_content = chatondesk_get_option('ninja_sms_body_' . $form_data['form_id'], 'chatondesk_ninja_message', '');
                    do_action('cod_send_sms', $billing_phone, self::parseSmsContent($buyer_sms_content, $datas));
                }
            }
            ...
            if ('on' === $admin_sms_notify ) {
                ...
                if (! empty($admin_phone_number) ) {
                    $admin_sms_content = chatondesk_get_option('ninja_admin_sms_body_' . $form_data['form_id'], 'chatondesk_ninja_message', '');
