 ...
 */
class SA_Easyappointments extends \ChatOnDesk\FormInterface
{
    /**
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::WP_EASY_APPOINTMENTS;
    ...
    {
        if (chatondesk_get_option('otp_enable', 'chatondesk_eap_general') === 'on') {
            echo do_shortcode('[cod_verify phone_selector="#phone" submit_selector= ".ea-submit"]');
        }
    }
    ...
    {  
        ...
        $customerNotify  = chatondesk_get_option('customer_notify', 'chatondesk_eap_general', 'on');
        global $wpdb;
    ...
    {    
        if (chatondesk_get_option('customer_notify', 'chatondesk_eap_general') !=='on') {
            return;
        }
        ...
        $schedulerData   = get_option('chatondesk_eap_reminder_scheduler');
    ...
    {   
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $defaults['chatondesk_eap_general']['customer_eap_notify_' . $vs] = 'off';
            $defaults['chatondesk_eap_message']['customer_sms_eap_body_' . $vs] = '';
            $defaults['chatondesk_eap_general']['admin_eap_notify_' . $vs] = 'off';
            $defaults['chatondesk_eap_message']['admin_sms_eap_body_' . $vs]    = '';
        }
        $defaults['chatondesk_eap_general']['otp_enable']                      = 'off';
        $defaults['chatondesk_eap_general']['customer_notify']                 = 'off';
        $defaults['chatondesk_eap_reminder_scheduler']['cron'][0]['frequency'] = '1';
        $defaults['chatondesk_eap_reminder_scheduler']['cron'][0]['message']   = '';
        return $defaults;
    }
    ...
    {
        $currentVal     = chatondesk_get_option('customer_notify', 'chatondesk_eap_general', 'on');
        $checkboxNameId = 'chatondesk_eap_general[customer_notify]';
        $schedulerData  = get_option('chatondesk_eap_reminder_scheduler');
        $templates      = array();
        $count          = 0;
        if (empty($schedulerData) === true) {
            $schedulerData['cron'][] = array(
                'frequency' => '1',
                'message'   => sprintf(__('Hello %1$s, your appointment %2$s with %3$s is fixed on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[name]', '#[id]', '[store_name]', '[bookingdate]', PHP_EOL, PHP_EOL),
            );
        }
        foreach ($schedulerData['cron'] as $key => $data) {
            $textAreaNameId  = 'chatondesk_eap_reminder_scheduler[cron][' . $count . '][message]';
            $selectNameId    = 'chatondesk_eap_reminder_scheduler[cron][' . $count . '][frequency]';
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates           = [];
        foreach ($bookingStatuses as $ks => $vs) {
            $currentVal      = chatondesk_get_option('customer_eap_notify_' . strtolower($vs), 'chatondesk_eap_general', 'on');
            $checkboxNameId  = 'chatondesk_eap_general[customer_eap_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_eap_message[customer_sms_eap_body_' . strtolower($vs) . ']';

            $defaultTemplate = chatondesk_get_option('customer_sms_eap_body_' . strtolower($vs), 'chatondesk_eap_message', sprintf(__('Hello %1$s, status of your appointment #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[name]', '[id]', '[store_name]', $vs, PHP_EOL, PHP_EOL));

            $textBody       = chatondesk_get_option('customer_sms_eap_body_' . strtolower($vs), 'chatondesk_eap_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            ...
            $currentVal      = chatondesk_get_option('admin_eap_notify_' . strtolower($vs), 'chatondesk_eap_general', 'on');
            $checkboxNameId  = 'chatondesk_eap_general[admin_eap_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_eap_message[admin_sms_eap_body_' . strtolower($vs) . ']';

            $defaultTemplate = chatondesk_get_option('admin_sms_eap_body_' . strtolower($vs), 'chatondesk_eap_message', sprintf(__('Hello admin, status of your appointment with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $vs, PHP_EOL, PHP_EOL));


            $textBody = chatondesk_get_option('admin_sms_eap_body_' . strtolower($vs), 'chatondesk_eap_message', $defaultTemplate);
    ...
    public function sendSmsOn($appointmentId,$data)
    {
        ...
        if (!empty($buyerNumber)) {
            $customerMessage   = chatondesk_get_option('customer_sms_eap_body_' . $bookingStatus, 'chatondesk_eap_message', '');
            
            $customerNotify    = chatondesk_get_option('customer_eap_notify_' . $bookingStatus, 'chatondesk_eap_general', 'on');
            ...
            if (empty($adminPhoneNumber) === false) {
                $adminNotify        = chatondesk_get_option('admin_eap_notify_' . $bookingStatus, 'chatondesk_eap_general', 'on');
                $adminMessage       = chatondesk_get_option('admin_sms_eap_body_' . $bookingStatus, 'chatondesk_eap_message', '');
