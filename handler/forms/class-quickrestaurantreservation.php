 ...
 */
class Quickrestaurantreservation extends \ChatOnDesk\FormInterface
{
    /**
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::WP_QUICK_RES_RESERVATION;
    ...
    {
        if (chatondesk_get_option('otp_enable', 'chatondesk_qr_general') === 'on') {
            echo do_shortcode('[cod_verify phone_selector="#qrr-phone" submit_selector= ".qrr-submit button"]');
        }
    }
    ...
    public static function setBookingReminder($booking)
    {
        ...
        $customerNotify  = chatondesk_get_option('customer_notify', 'chatondesk_qr_general', 'on');
        global $wpdb;
    ...
    {
        if (chatondesk_get_option('customer_notify', 'chatondesk_qr_general') !== 'on') {
            return;
        }
        ...
        $tableName       = $wpdb->prefix . 'chatondesk_booking_reminder';
        $source          = 'quick-restaurant-reservations';
        $schedulerData   = get_option('chatondesk_qr_reminder_scheduler');
    ...
    public static function add_default_setting($defaults = array())
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $defaults['chatondesk_qr_general']['customer_qr_notify_' . $ks]   = 'off';
            $defaults['chatondesk_qr_message']['customer_sms_qr_body_' . $ks] = '';
            $defaults['chatondesk_qr_general']['admin_qr_notify_' . $ks]      = 'off';
            $defaults['chatondesk_qr_message']['admin_sms_qr_body_' . $ks]    = '';
        }
        $defaults['chatondesk_qr_general']['otp_enable']                      = 'off';
        $defaults['chatondesk_qr_general']['customer_notify']                 = 'off';
        $defaults['chatondesk_qr_reminder_scheduler']['cron'][0]['frequency'] = '1';

        $defaults['chatondesk_qr_reminder_scheduler']['cron'][0]['message']   = '';
        return $defaults;
    }
    ...
    {
        $currentVal     = chatondesk_get_option('customer_notify', 'chatondesk_qr_general', 'on');
        $checkboxNameId = 'chatondesk_qr_general[customer_notify]';
        ...
        if (empty($schedulerData) === true) {
            $schedulerData['cron'][] = array(
                'frequency' => '1',
                'message'   => sprintf(__('Hello %1$s, your booking %2$s with %3$s is fixed on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[name]', '#[booking_id]', '[store_name]', '[booking_date]', PHP_EOL, PHP_EOL),
            );
        }
        foreach ($schedulerData['cron'] as $key => $data) {
            $textAreaNameId  = 'chatondesk_qr_reminder_scheduler[cron][' . $count . '][message]';
            $selectNameId    = 'chatondesk_qr_reminder_scheduler[cron][' . $count . '][frequency]';
    ...
    public static function getCustomerTemplates()
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            ...
            $currentVal      = chatondesk_get_option('customer_qr_notify_' . strtolower($vs), 'chatondesk_qr_general', 'on');



            $checkboxNameId  = 'chatondesk_qr_general[customer_qr_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_qr_message[customer_sms_qr_body_' . strtolower($vs) . ']';

            $defaultTemplate = chatondesk_get_option('customer_sms_qr_body_' . strtolower($vs), 'chatondesk_qr_message', sprintf(__('Hello %1$s, status of your booking #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[name]', '[booking_id]', '[store_name]', $label, PHP_EOL, PHP_EOL));


            $textBody       = chatondesk_get_option('customer_sms_qr_body_' . strtolower($vs), 'chatondesk_qr_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            ...
            $currentVal      = chatondesk_get_option('admin_qr_notify_' . strtolower($vs), 'chatondesk_qr_general', 'on');
            $checkboxNameId  = 'chatondesk_qr_general[admin_qr_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_qr_message[admin_sms_qr_body_' . strtolower($vs) . ']';

            $defaultTemplate = chatondesk_get_option('admin_sms_qr_body_' . strtolower($vs), 'chatondesk_qr_message', sprintf(__('Hello admin, status of your booking with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $label, PHP_EOL, PHP_EOL));


            $textBody = chatondesk_get_option('admin_sms_qr_body_' . strtolower($vs), 'chatondesk_qr_message', $defaultTemplate);
    ...
    public function sendSmsOn($post_id, $from, $to)
    {
        ...
        $customerMessage   = chatondesk_get_option('customer_sms_qr_body_' . $bookingStatus, 'chatondesk_qr_message', '');
        $customerNotify    = chatondesk_get_option('customer_qr_notify_' . $bookingStatus, 'chatondesk_qr_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminNotify        = chatondesk_get_option('admin_qr_notify_' . $bookingStatus, 'chatondesk_qr_general', 'on');
            $adminMessage       = chatondesk_get_option('admin_sms_qr_body_' . $bookingStatus, 'chatondesk_qr_message', '');
