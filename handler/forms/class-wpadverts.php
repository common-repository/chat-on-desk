 ...
 * SA_WPadverts class 
 */
class SA_WPadverts extends \ChatOnDesk\FormInterface
{
    /**
     * Construct function.
     *
    ...
    {  
        $customerNotify  = chatondesk_get_option('customer_notify', 'chatondesk_adv_general', 'on'); 
        $source          = 'wpadverts';
        ...
        if ('publish' === $status && 'on' === $customerNotify && $expiry) {
            $scheduler_data = get_option('chatondesk_adv_renewal_scheduler');
    ...
    {    
        ...
        $source               = 'wpadverts';
        $table_name           = $wpdb->prefix . 'chatondesk_renewal_reminders'; 
        $schedulerData        = get_option('chatondesk_adv_reminder_scheduler');
        $today                = new \DateTime();
    ...
    {  
        ...
        foreach ($adverdStatuses as $ks => $vs) {
            $defaults['chatondesk_adv_general']['customer_adv_notify_' . $vs]   = 'off';
            $defaults['chatondesk_adv_message']['customer_sms_adv_body_' . $vs] = '';
            $defaults['chatondesk_adv_general']['admin_adv_notify_' . $vs]      = 'off';
            $defaults['chatondesk_adv_message']['admin_sms_adv_body_' . $vs]    = '';
        }
        $defaults['chatondesk_adv_general']['author_adv_notify_message']   = 'off';
        $defaults['chatondesk_adv_message']['author_sms_adv_body_message'] = '';
        $defaults['chatondesk_adv_renewal']['customer_notify']                  = 'off';
        $defaults['chatondesk_adv_renewal_scheduler']['cron'][0]['frequency']   = '1';
        $defaults['chatondesk_adv_renewal_scheduler']['cron'][0]['message']     = '';
        $defaults['chatondesk_adv_renewal_scheduler']['cron'][1]['frequency']   = '2';
        $defaults['chatondesk_adv_renewal_scheduler']['cron'][1]['message']     = '';
        return $defaults;
    }
    ...
    {
        $currentVal     = chatondesk_get_option('customer_notify', 'chatondesk_adv_renewal', 'on');
        $checkboxNameId = 'chatondesk_adv_renewal[customer_notify]';
        $schedulerData  = get_option('chatondesk_adv_renewal_scheduler');
        $templates      = array();
        $count          = 0;
        if (empty($schedulerData) === true) {
            $schedulerData['cron'][] = array(
                'frequency' => '1',
                'message'   => sprintf(__('Hello %1$s, your advertisement %2$s with %3$s is expired on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[authorName]', '#[id]', '[store_name]', '[expiration_date]', PHP_EOL, PHP_EOL),
            );
            $schedulerData['cron'][] = array(
                'frequency' => '2',
                'message'   => sprintf(__('Hello %1$s, your advertisement %2$s with %3$s is expired on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[authorName]', '#[id]', '[store_name]', '[expiration_date]', PHP_EOL, PHP_EOL),
        ...
        foreach ($schedulerData['cron'] as $key => $data) {
            ...
            $textAreaNameId  = 'chatondesk_adv_renewal_scheduler[cron][' . $count . '][message]';
            $selectNameId    = 'chatondesk_adv_renewal_scheduler[cron][' . $count . '][frequency]';
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates          = array();
        foreach ($adverdStatuses as $ks => $vs) {
            $currentVal      = chatondesk_get_option('customer_adv_notify_' . strtolower($vs), 'chatondesk_adv_general', 'on');
            $checkboxNameId  = 'chatondesk_adv_general[customer_adv_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_adv_message[customer_sms_adv_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('customer_sms_adv_body_' . strtolower($vs), 'chatondesk_adv_message', sprintf(__('Hello %1$s, status of your advertisement #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[authorName]', '[id]', '[store_name]', $vs, PHP_EOL, PHP_EOL));
            $textBody       = chatondesk_get_option('customer_sms_adv_body_' . strtolower($vs), 'chatondesk_adv_message', $defaultTemplate);
    ...
    public static function getAuthorTemplates()
    {
        ...
        foreach ($authorStatuses as $ks => $vs) {
            $currentVal      = chatondesk_get_option('author_adv_notify_' . strtolower($vs), 'author_adv_general', 'on');
            $checkboxNameId  = 'chatondesk_adv_general[author_adv_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_adv_message[author_sms_adv_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('author_sms_adv_body_' . strtolower($vs), 'chatondesk_adv_message', SmsAlertMessages::showMessage('DEFAULT_CONTACT_FORM_ADMIN_MESSAGE'));
            $textBody       = chatondesk_get_option('author_sms_adv_body_' . strtolower($vs), 'chatondesk_adv_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
         $adverdStatuses     = array('pending', 'publish', 'expired', 'trash', );
          $templates           =array();
        foreach ($adverdStatuses as $ks => $vs) {
            $currentVal      = chatondesk_get_option('admin_adv_notify_' . strtolower($vs), 'chatondesk_adv_general', 'on');
            $checkboxNameId  = 'chatondesk_adv_general[admin_adv_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_adv_message[admin_sms_adv_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('admin_sms_adv_body_' . strtolower($vs), 'chatondesk_adv_message', sprintf(__('%1$s status of advertisement has been changed to %2$s.', 'chat-on-desk'), '[store_name]:', $vs));
            $textBody = chatondesk_get_option('admin_sms_adv_body_' . strtolower($vs), 'chatondesk_adv_message', $defaultTemplate);
    ...
    public function sendSmsAuthor($post_id, $form)
    {
        ...
        if (!empty($authorNumber)) {
            $authorMessage = chatondesk_get_option('author_sms_adv_body_message', 'chatondesk_adv_message', '');
            $authorNotify  = chatondesk_get_option('author_adv_notify_message', 'chatondesk_adv_general', 'on');
            if (($authorNotify === 'on' && $authorMessage !== '')) {
                $buyerMessage = $this->parseSmsBody($post, $authorMessage);
                do_action('cod_send_sms', $authorNumber, $buyerMessage);
            }
        } 
    }
    ...
    public function advertsPostUpdated($post)
    {
        if (get_post_meta($post->ID, "_adverts_payment_type", true) == "adverts-renewal" ) {
            ...
            global $wpdb;
            $tableName       = $wpdb->prefix . 'chatondesk_renewal_reminders';
            $source          = 'wpadverts';
            $wpdb->update(
                $tableName,
                array(
    ...
    public function sendSmsOn($post)
    {
        ...
        if (!empty($authorNumber)) {
            $customerMessage = chatondesk_get_option('customer_sms_adv_body_' . $status, 'chatondesk_adv_message', '');
            $customerNotify  = chatondesk_get_option('customer_adv_notify_' . $status, 'chatondesk_adv_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminNotify        = chatondesk_get_option('admin_adv_notify_' . $status, 'chatondesk_adv_general', 'on');
            $adminMessage       = chatondesk_get_option('admin_sms_adv_body_' . $status, 'chatondesk_adv_message', '');
