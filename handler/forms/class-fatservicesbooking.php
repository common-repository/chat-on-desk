 ...
 */
class FatServiceBooking extends \ChatOnDesk\FormInterface
{
    ...
    {       
        ...
        $customerNotify = chatondesk_get_option('customer_notify', 'chatondesk_fsb_general', 'on');
        global $wpdb;
    ...
    {
        if (chatondesk_get_option('customer_notify', 'chatondesk_fsb_general') !== 'on') {
            return;
        }
        global $wpdb;
        ...
        $source        = 'fat_services_booking';
        $schedulerData = get_option('chatondesk_fsb_reminder_scheduler');
    ...
    public static function add_default_setting($defaults = array())
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $defaults['chatondesk_fsb_general']['customer_fsb_notify_' . $vs]   = 'off';
            $defaults['chatondesk_fsb_message']['customer_sms_fsb_body_' . $vs] = '';
            $defaults['chatondesk_fsb_general']['admin_fsb_notify_' . $vs]      = 'off';
            $defaults['chatondesk_fsb_message']['admin_sms_fsb_body_' . $vs]    = '';
        }
        $defaults['chatondesk_fsb_reminder_scheduler']['cron'][0]['frequency'] = '1';
        $defaults['chatondesk_fsb_reminder_scheduler']['cron'][0]['message']   = '';
        return $defaults;
    ...
    {
        $currentVal      = chatondesk_get_option('customer_notify', 'chatondesk_fsb_general', 'on');
        $checkboxNameId  = 'chatondesk_fsb_general[customer_notify]';
        $schedulerData  = get_option('chatondesk_fsb_reminder_scheduler');
        $templates      = array();
        $count          = 0;
        if (empty($schedulerData) === true) {
            $schedulerData['cron'][] = array(
                'frequency' => '1',
                'message'   => sprintf(__('Hello %1$s, your booking %2$s with %3$s is fixed on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[c_first_name]', '#[b_id]', '[store_name]', '[b_date]', PHP_EOL, PHP_EOL),
            );
        }
        foreach ($schedulerData['cron'] as $key => $data) {
            $textAreaNameId = 'chatondesk_fsb_reminder_scheduler[cron][' . $count . '][message]';
            $selectNameId    = 'chatondesk_fsb_reminder_scheduler[cron][' . $count . '][frequency]';
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates = array();
        foreach ($bookingStatuses as $ks  => $vs) {
            $currentVal = chatondesk_get_option('customer_fsb_notify_' . strtolower($vs), 'chatondesk_fsb_general', 'on');
            $checkboxNameId = 'chatondesk_fsb_general[customer_fsb_notify_' . strtolower($vs) . ']';
            $textareaNameId = 'chatondesk_fsb_message[customer_sms_fsb_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('admin_sms_fsb_body_' . strtolower($vs), 'chatondesk_fsb_message', sprintf(__('Hello %1$s, status of your booking #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[c_first_name]', '[b_id]', '[store_name]', strtolower($vs), PHP_EOL, PHP_EOL));
            $textBody = chatondesk_get_option('customer_sms_fsb_body_' . strtolower($vs), 'chatondesk_fsb_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        $templates = array();
        foreach ($bookingStatuses as $ks  => $vs) {
            $currentVal     = chatondesk_get_option('admin_fsb_notify_' . strtolower($vs), 'chatondesk_fsb_general', 'on');
            $checkboxNameId = 'chatondesk_fsb_general[admin_fsb_notify_' . strtolower($vs) . ']';
            $textareaNameId = 'chatondesk_fsb_message[admin_sms_fsb_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('admin_sms_fsb_body_' . strtolower($vs), 'chatondesk_fsb_message', sprintf(__('Hello admin, status of your booking with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', strtolower($vs), PHP_EOL, PHP_EOL));
            $textBody = chatondesk_get_option('admin_sms_fsb_body_' . strtolower($vs), 'chatondesk_fsb_message', $defaultTemplate);
    ...
    public function sendSmsOn($booking_id, $booking)
    {
        ...
        $bookingStatus   = self::$b_status[$booking->b_process_status];      
        $customerMessage = chatondesk_get_option('customer_sms_fsb_body_' . $bookingStatus, 'chatondesk_fsb_message', '');        
        $customerNotify = chatondesk_get_option('customer_fsb_notify_' . $bookingStatus, 'chatondesk_fsb_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            ...
            $adminNotify  = chatondesk_get_option('admin_fsb_notify_' . $bookingStatus, 'chatondesk_fsb_general', 'on');

            $adminMessage = chatondesk_get_option('admin_sms_fsb_body_' . $bookingStatus, 'chatondesk_fsb_message', '');
