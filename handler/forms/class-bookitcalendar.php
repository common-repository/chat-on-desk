 ...
 */
class BookitCalendar extends \ChatOnDesk\FormInterface
{
    ...
    public static function setBookingReminder($appointment)
    {
        ...
        $customerNotify = chatondesk_get_option('customer_notify', 'chatondesk_bcc_general', 'on');
        global $wpdb;
    ...
    {
        if (chatondesk_get_option('customer_notify', 'chatondesk_bcc_general') !== 'on') {
            return;
        }
        ...
        $tableName     = $wpdb->prefix.'chatondesk_booking_reminder';
        $source        = 'bookit-calendar';
        $schedulerData = get_option('chatondesk_bcc_reminder_scheduler');
    ...
    public static function add_default_setting($defaults=[])
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $defaults['chatondesk_bcc_general']['customer_bcc_notify_'.$vs]   = 'off';
            $defaults['chatondesk_bcc_message']['customer_sms_bcc_body_'.$vs] = '';
            $defaults['chatondesk_bcc_general']['admin_bcc_notify_'.$vs]      = 'off';
            $defaults['chatondesk_bcc_message']['admin_sms_bcc_body_'.$vs]    = '';
        }

        $defaults['chatondesk_bcc_general']['otp_enable']      = 'off';
        $defaults['chatondesk_bcc_general']['customer_notify'] = 'off';
        $defaults['chatondesk_bcc_reminder_scheduler']['cron'][0]['frequency'] = '1';
        $defaults['chatondesk_bcc_reminder_scheduler']['cron'][0]['message']   = '';
        return $defaults;
    ...
    {
        $currentVal     = chatondesk_get_option('customer_notify', 'chatondesk_bcc_general', 'on');
        $checkboxMameId = 'chatondesk_bcc_general[customer_notify]';
        ...
        if (empty($schedulerData) === true) {
            $schedulerData['cron'][] = [
                'frequency' => '1',
                'message'   => sprintf(__('Hello %1$s, your booking %2$s with %3$s is fixed on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[customer_name]', '#[id]', '[store_name]', '[start_time]', PHP_EOL, PHP_EOL),
            ];
        }

        foreach ($schedulerData['cron'] as $key => $data) {
            $textAreaNameId = 'chatondesk_bcc_reminder_scheduler[cron]['.$count.'][message]';
            $selectNameId   = 'chatondesk_bcc_reminder_scheduler[cron]['.$count.'][frequency]';
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates = [];
        foreach ($bookingStatuses as $ks  => $vs) {
            $currentVal = chatondesk_get_option('customer_bcc_notify_'.strtolower($vs), 'chatondesk_bcc_general', 'on');

            $checkboxMameId = 'chatondesk_bcc_general[customer_bcc_notify_'.strtolower($vs).']';
            $textareaNameId = 'chatondesk_bcc_message[customer_sms_bcc_body_'.strtolower($vs).']';

            $defaultTemplate = sprintf(__('Hello %1$s, status of your booking #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[customer_name]', '[id]', '[store_name]', $vs, PHP_EOL, PHP_EOL);

            $textBody = chatondesk_get_option('customer_sms_bcc_body_'.strtolower($vs), 'chatondesk_bcc_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        $templates = [];
        foreach ($bookingStatuses as $ks  => $vs) {
            $currentVal     = chatondesk_get_option('admin_bcc_notify_'.strtolower($vs), 'chatondesk_bcc_general', 'on');
            $checkboxMameId = 'chatondesk_bcc_general[admin_bcc_notify_'.strtolower($vs).']';
            $textareaNameId = 'chatondesk_bcc_message[admin_sms_bcc_body_'.strtolower($vs).']';

            $defaultTemplate = sprintf(__('Hello admin, status of your booking with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $vs, PHP_EOL, PHP_EOL);

            $textBody = chatondesk_get_option('admin_sms_bcc_body_'.strtolower($vs), 'chatondesk_bcc_message', $defaultTemplate);
    ...
    public function sendsmsNewBookit($app_id)
    {
        ...
        $customerMessage   = chatondesk_get_option('customer_sms_bcc_body_pending', 'chatondesk_bcc_message', '');
        $customerBccNotify = chatondesk_get_option('customer_bcc_notify_pending', 'chatondesk_bcc_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminBccNotify = chatondesk_get_option('admin_bcc_notify_pending', 'chatondesk_bcc_general', 'on');
            $adminMessage   = chatondesk_get_option('admin_sms_bcc_body_pending', 'chatondesk_bcc_message', '');
    ...
    public function sendsmsBookitStatusUpdated($app_id)
    {
        ...
        $customerMessage = chatondesk_get_option('customer_sms_bcc_body_'.$bookingStatus, 'chatondesk_bcc_message', '');

        $customerNotify = chatondesk_get_option('customer_bcc_notify_'.$bookingStatus, 'chatondesk_bcc_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminNotify = chatondesk_get_option('admin_bcc_notify_'.$bookingStatus, 'chatondesk_bcc_general', 'on');

            $adminMessage = chatondesk_get_option('admin_sms_bcc_body_'.$bookingStatus, 'chatondesk_bcc_message', '');
