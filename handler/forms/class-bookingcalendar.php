 ...
 */
class BookingCalendar extends \ChatOnDesk\FormInterface
{
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::BC_FORM;
    ...
    public static function setBookingReminder( $booking_id )
    {
        if (function_exists('wpbc_api_get_booking_by_id') ) {
            ...
            $customer_notify = chatondesk_get_option('customer_notify', 'chatondesk_bc_general', 'on');
            global $wpdb;
    ...
    {
        if ('on' !==chatondesk_get_option('customer_notify', 'chatondesk_bc_general')) {
            return;
        }
        ...
        $table_name     = $wpdb->prefix . 'chatondesk_booking_reminder';
        $source = 'booking-calendar';
        $scheduler_data = get_option('chatondesk_bc_reminder_scheduler');
    ...
    public function getFormField($form , $my_boook_type)
    {
        ...
        if ('on' === chatondesk_get_option('checkout_show_country_code', 'chatondesk_general') &&  'off' === chatondesk_get_option('otp_enable', 'chatondesk_bc_general') && ( 'on' === chatondesk_get_option('customer_bc_notify_pending', 'chatondesk_bc_general') || 'on' === chatondesk_get_option('customer_bc_notify_approved', 'chatondesk_bc_general') || 'on' === chatondesk_get_option('customer_bc_notify_trash', 'chatondesk_bc_general') )) {
            $form .= '<script>
				jQuery("#phone1").addClass("phone-valid");
				</script>';
        }
        if ('on' === chatondesk_get_option('checkout_show_country_code', 'chatondesk_general') ||  'on' === chatondesk_get_option('otp_enable', 'chatondesk_bc_general')) {
            $form .= '<script>
        ...
        if ('on' === chatondesk_get_option('otp_enable', 'chatondesk_bc_general')) {

            $form .= do_shortcode('[cod_verify phone_selector="#phone1" submit_selector= ".btn-default"]');
            $form .= '<script>			
				setTimeout(function (){jQuery(".cod-otp-btn-init").removeAttr("onclick")}, 1000);
			</script>';
        }
        return($form);
    }
    ...
    public static function add_default_setting( $defaults = array() )
    {
        ...
        foreach ( $booking_statuses as $ks => $vs ) {
            $defaults['chatondesk_bc_general'][ 'customer_bc_notify_' . $vs ] = 'off';
            $defaults['chatondesk_bc_message'][ 'customer_sms_bc_body_' . $vs ] = '';
            $defaults['chatondesk_bc_general'][ 'admin_bc_notify_' . $vs ] = 'off';
            $defaults['chatondesk_bc_message'][ 'admin_sms_bc_body_' . $vs ] = '';
        }
        $defaults['chatondesk_bc_general'][ 'otp_enable'] = 'off';
        $defaults['chatondesk_bc_general']['customer_notify']                = 'off';
        $defaults['chatondesk_bc_reminder_scheduler']['cron'][0]['frequency'] = '1';
        $defaults['chatondesk_bc_reminder_scheduler']['cron'][0]['message']   = '';
        return $defaults;
    }
    ...
    {
        $current_val      = chatondesk_get_option('customer_notify', 'chatondesk_bc_general', 'on');
        $checkbox_name_id = 'chatondesk_bc_general[customer_notify]';
        ...
        foreach ( $scheduler_data['cron'] as $key => $data ) {
            ...
            $text_area_name_id = 'chatondesk_bc_reminder_scheduler[cron][' . $count . '][message]';
            $select_name_id    = 'chatondesk_bc_reminder_scheduler[cron][' . $count . '][frequency]';
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates = array();
        foreach ( $booking_statuses as $ks  => $vs ) {
            $current_val = chatondesk_get_option('customer_bc_notify_' . strtolower($vs), 'chatondesk_bc_general', 'on');

            $checkbox_name_id = 'chatondesk_bc_general[customer_bc_notify_' . strtolower($vs) . ']';
            $textarea_name_id = 'chatondesk_bc_message[customer_sms_bc_body_' . strtolower($vs) . ']';

            $default_template = SmsAlertMessages::showMessage('DEFAULT_BOOKING_CALENDAR_CUSTOMER_' . strtoupper($vs));

            $text_body = chatondesk_get_option('customer_sms_bc_body_' . strtolower($vs), 'chatondesk_bc_message', ( ( '' !== $default_template ) ? $default_template : SmsAlertMessages::showMessage('DEFAULT_BOOKING_CALENDAR_CUSTOMER') ));
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ( $booking_statuses as $ks  => $vs ) {
            ...
            $current_val      = chatondesk_get_option('admin_bc_notify_' . strtolower($vs), 'chatondesk_bc_general', 'on');
            $checkbox_name_id = 'chatondesk_bc_general[admin_bc_notify_' . strtolower($vs) . ']';
            $textarea_name_id = 'chatondesk_bc_message[admin_sms_bc_body_' . strtolower($vs) . ']';

            /* translators: %1$s: Store name tag, %2$s: Booking status */
            $text_body = chatondesk_get_option('admin_sms_bc_body_' . strtolower($vs), 'chatondesk_bc_message', sprintf(__('Hello admin, status of your booking with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $vs, PHP_EOL, PHP_EOL));
    ...
    public function sendsmsNewBooking( $booking_id )
    {
        if (function_exists('wpbc_api_get_booking_by_id') ) {
            ...
            $customer_message   = chatondesk_get_option('customer_sms_bc_body_pending', 'chatondesk_bc_message', '');
            $customer_bc_notify = chatondesk_get_option('customer_bc_notify_pending', 'chatondesk_bc_general', 'on');

            if ('on' === $customer_bc_notify && '' !== $customer_message ) {
                $buyer_message = $this->parseSmsBody($booking, $customer_message);
                do_action('cod_send_sms', $buyer_number, $buyer_message);
            }
            ...
            if (! empty($admin_phone_number) ) {
                ...
                $admin_bc_notify = chatondesk_get_option('admin_bc_notify_pending', 'chatondesk_bc_general', 'on');
                $admin_message   = chatondesk_get_option('admin_sms_bc_body_pending', 'chatondesk_bc_message', '');

                if ('on' === $admin_bc_notify && '' !== $admin_message ) {
                    $admin_message = $this->parseSmsBody($booking, $admin_message);
                    do_action('cod_send_sms', $admin_phone_number, $admin_message);
                }
            }
        }
    }
    ...
    public function sendsmsApprovedPending( $booking_id, $is_approve_or_pending )
    {
        ...
        if (function_exists('wpbc_api_get_booking_by_id') ) {
            ...
            if ('1' === $is_approve_or_pending ) {
                $customer_message = chatondesk_get_option('customer_sms_bc_body_approved', 'chatondesk_bc_message', '');
                 self::setBookingReminder($booking_id);
            } else {
                $customer_message = chatondesk_get_option('customer_sms_bc_body_pending', 'chatondesk_bc_message', '');
            }

            $customer_bc_pending_notify  = chatondesk_get_option('customer_bc_notify_pending', 'chatondesk_bc_general', 'on');
            $customer_bc_approved_notify = chatondesk_get_option('customer_bc_notify_approved', 'chatondesk_bc_general', 'on');
            ...
            if (! empty($admin_phone_number) ) {
                ...
                $chatondesk_bc_admin_pending_notify = chatondesk_get_option('admin_bc_notify_pending', 'chatondesk_bc_general', 'on');
                $chatondesk_bc_admin_approve_notify = chatondesk_get_option('admin_bc_notify_approved', 'chatondesk_bc_general', 'on');

                if ('1' === $is_approve_or_pending ) {
                    $admin_message = chatondesk_get_option('admin_sms_bc_body_approved', 'chatondesk_bc_message', '');
                } else {
                    $admin_message = chatondesk_get_option('admin_sms_bc_body_pending', 'chatondesk_bc_message', '');
                }
                ...
                if ('on' === $chatondesk_bc_admin_pending_notify && '' !== $admin_message && '0' === $is_approve_or_pending ) {
                    $admin_message = $this->parseSmsBody($booking, $admin_message);
                    do_action('cod_send_sms', $admin_phone_number, $admin_message);
                }

                if ('on' === $chatondesk_bc_admin_approve_notify && '' !== $admin_message && '1' === $is_approve_or_pending ) {
                    $admin_message = $this->parseSmsBody($booking, $admin_message);
                    do_action('cod_send_sms', $admin_phone_number, $admin_message);
                }
            }
    ...
    public function sendsmsTrash( $booking_id, $is_trash )
    {
        ...
        if (function_exists('wpbc_api_get_booking_by_id') ) {
            ...
            $customer_message   = chatondesk_get_option('customer_sms_bc_body_trash', 'chatondesk_bc_message', '');
            $customer_bc_notify = chatondesk_get_option('customer_bc_notify_trash', 'chatondesk_bc_general', 'on');
            ...
            if (! empty($admin_phone_number) ) {
                ...
                $admin_bc_notify = chatondesk_get_option('admin_bc_notify_trash', 'chatondesk_bc_general', 'on');
                $admin_message   = chatondesk_get_option('admin_sms_bc_body_trash', 'chatondesk_bc_message', '');
