...
use \ChatOnDesk\FormInterface;
use chatondesk_Setting_Options;
use \ChatOnDesk\FormSessionVars;
use \ChatOnDesk\SmsAlertUtility;
use \ChatOnDesk\SmsAlertMessages;
 ...
 * Armember class 
 */
class Armember extends \ChatOnDesk\FormInterface
{
    /**
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::AR_MEMBER_FORM;
    ...
    public function addLoginOtp($field_content, $form)
    {
        ...
        if ($form->type=='login' && 'on' ===$default_login_otp) {                
            $field_content.= do_shortcode('[cod_verify user_selector="user_login" pwd_selector="user_pass" submit_selector=".arm_form_field_submit_button"]');    
        }
    ...
    public function addPhoneField($content, $form)
    {
        if ($form->type == 'registration') {
            ...
            if ('on' === $buyer_signup_otp ) {                
                $content.=do_shortcode('[cod_verify phone_selector="billing_phone" submit_selector= ".arm_form_field_submit_button"]');    
            }            
        }
        return $content;
    }
    ...
    public static function add_default_setting($defaults = array())
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $defaults['chatondesk_arm_general']['customer_arm_notify_' . $vs] = 'off';
            $defaults['chatondesk_arm_message']['customer_sms_arm_body_' . $vs] = '';
            $defaults['chatondesk_arm_general']['admin_arm_notify_' . $vs]    = 'off';
            $defaults['chatondesk_arm_message']['admin_sms_arm_body_' . $vs]  = '';
        }
        return $defaults;
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates = array();
        foreach ($bookingStatuses as $ks  => $vs) {
            $currentVal = chatondesk_get_option('customer_arm_notify_' . strtolower($vs), 'chatondesk_arm_general', 'on');

            $checkboxNameId = 'chatondesk_arm_general[customer_arm_notify_' . strtolower($vs) . ']';
            $textareaNameId = 'chatondesk_arm_message[customer_sms_arm_body_' . strtolower($vs) . ']';

            $defaultTemplate = chatondesk_get_option('admin_sms_arm_body_' . strtolower($vs), 'chatondesk_arm_message', sprintf(__('Hello %1$s, status of your plan %2$s with %3$s has been %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[member_name]', '[plan_name]', '[store_name]', $vs, PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_arm_body_' . strtolower($vs), 'chatondesk_arm_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ($bookingStatuses as $ks  => $vs) {
            ...
            $currentVal     = chatondesk_get_option('admin_arm_notify_' . strtolower($vs), 'chatondesk_arm_general', 'on');
            $checkboxNameId = 'chatondesk_arm_general[admin_arm_notify_' . strtolower($vs) . ']';
            $textareaNameId = 'chatondesk_arm_message[admin_sms_arm_body_' . strtolower($vs) . ']';

            $defaultTemplate = chatondesk_get_option('admin_sms_arm_body_' . strtolower($vs), 'chatondesk_arm_message', sprintf(__('Hello admin, status of your plan %1$s with %2$s has been changed to %3$s. %4$sPowered by%5$swww.chatondesk.com', 'chat-on-desk'), '[plan_name]', '[store_name]', $vs, PHP_EOL, PHP_EOL));


            $textBody = chatondesk_get_option('admin_sms_arm_body_' . strtolower($vs), 'chatondesk_arm_message', $defaultTemplate);
    ...
    public function sendSmsOnStatusAfterUserPlanRenew($user_id, $plan_id)
    {
        ...
        $buyerSmsData = array();                  
        $customerMessage  = chatondesk_get_option('customer_sms_arm_body_renewed', 'chatondesk_arm_message', '');
        $customerRrNotify = chatondesk_get_option('customer_arm_notify_renewed', 'chatondesk_arm_general', 'on');
        if ($customerRrNotify === 'on' && $customerMessage !== '') {
            $buyerMessage = $this->parseSmsBody($planData, $user_id, $customerMessage);
            do_action('cod_send_sms', $user_phone, $buyerMessage);
        }
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminRrNotify = chatondesk_get_option('admin_arm_notify_renewed', 'chatondesk_arm_general', 'on');
            $adminMessage   = chatondesk_get_option('admin_sms_arm_body_renewed', 'chatondesk_arm_message', '');
    ...
    public function sendSmsOnStatusCancelSubscription($user_id, $plan_id)
    {
        ...
        $planData     = get_user_meta($user_id, "arm_user_plan_{$plan_id}", true);
        $buyerSmsData = array();                  
        $customerMessage  = chatondesk_get_option('customer_sms_arm_body_cancelled', 'chatondesk_arm_message', '');
        $customerRrNotify = chatondesk_get_option('customer_arm_notify_cancelled', 'chatondesk_arm_general', 'on');
        if ($customerRrNotify === 'on' && $customerMessage !== '') {
            $buyerMessage = $this->parseSmsBody($planData, $user_id, $customerMessage);
            do_action('cod_send_sms', $user_phone, $buyerMessage);
        }
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminRrNotify = chatondesk_get_option('admin_arm_notify_cancelled', 'chatondesk_arm_general', 'on');
            $adminMessage   = chatondesk_get_option('admin_sms_arm_body_cancelled', 'chatondesk_arm_message', '');
            if ('on' === $adminRrNotify && '' !== $adminMessage) {
                $adminMessage = $this->parseSmsBody($planData, $user_id, $adminMessage);
                do_action('cod_send_sms', $adminPhoneNumber, $adminMessage);
            }
    ...
    public function sendSmsOnStatusAfterUserPlanChange($user_id, $plan_id)
    {
        ...
        $customerMessage = chatondesk_get_option('customer_sms_arm_body_' . $subscriptionstatus, 'chatondesk_arm_message', '');
        $customerNotify = chatondesk_get_option('customer_arm_notify_' . $subscriptionstatus, 'chatondesk_arm_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminNotify  = chatondesk_get_option('admin_arm_notify_' . $subscriptionstatus, 'chatondesk_arm_general', 'on');
            $adminMessage = chatondesk_get_option('admin_sms_arm_body_' . $subscriptionstatus, 'chatondesk_arm_message', '');
            $nos = explode(',', $adminPhoneNumber);
            $adminPhoneNumber = array_diff($nos, array('postauthor', 'post_author'));
            $adminPhoneNumber = implode(',', $adminPhoneNumber);
