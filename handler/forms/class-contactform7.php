 ...
 * ContactForm7 class.
 */
class ContactForm7 extends \ChatOnDesk\FormInterface
{
     ...
     * @var stirng
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::CF7_FORMS;
     ...
     * @var stirng
     */
    private $form_phone_ver = \ChatOnDesk\FormSessionVars::CF7_PHONE_VER;
     ...
     * @var stirng
     */
    private $form_final_phone_ver = \ChatOnDesk\FormSessionVars::CF7_PHONE_SUB;
    ...
    public function handleForm()
    {
        ...
        add_filter('wpcf7_validate_chatondesk_otp_input*', array( $this, 'validateFormPost' ), 1, 2);
        add_shortcode('chatondesk_verify_phone', array( $this, 'cf7PhoneShortcode' ));
        $this->routeData();
    ...
    { 
        ...
        $id = $_POST['_wpcf7'];
        $options         = get_option('chatondesk_sms_c7_' . $id);
        $visitor_number = !empty($options['visitorNumber'])?$this->getCf7TagSToString($options['visitorNumber'], $_POST):'';
        if (isset($_REQUEST['option']) && 'chatondesk_wpcf7_form_otp' === sanitize_text_field(wp_unslash($_REQUEST['option']))) {
            \ChatOnDesk\SmsAlertUtility::initialize_transaction($this->form_session_var);
        } else {
            return $result;
        }        
           
        if (isset($visitor_number) && \ChatOnDesk\SmsAlertUtility::isBlank($visitor_number)) {            
    ...
    function smsalertWpcf7ShowWarnings($page,$action,$object)
    {
        ...
        if (!empty($_REQUEST['post'])) {
            $options         = get_option('chatondesk_sms_c7_' .$_REQUEST['post']);
            if (((!empty($options['visitor_notification']) && 'on' === $options['visitor_notification']) || (!empty($options['auto_sync']) && 'on' === $options['auto_sync'])) && (empty($options['visitorNumber']) || (!empty($options['visitorNumber']) && '[billing_phone]' !== $options['visitorNumber']))) {
                echo sprintf(
                    ...
                    esc_html__("Please choose Chat On Desk phone field in Chat On Desk tab", 'chat-on-desk')
                );
            }
        }
    }
    ...
    {
        wpcf7_add_form_tag(
            array( 'billing_phone', 'billing_phone*', 'chatondesk_otp_input', 'chatondesk_otp_input*' ),
    ...
    public function smsalertWpcf7ShortcodeHandler( $tag )
    {
        ...
        if ($tag->has_option('otp_enabled_popup') ) {
            $class .= ' wpcf7-chatondesk-otp-enabled phone-valid';
        } else {
            $class .= ' phone-valid';
        }
        ...
        if ($tag->has_option('otp_enabled_popup') ) {
            $html .= do_shortcode("[cod_verify phone_selector='.wpcf7-billing_phone' submit_selector='#" . $unit_tag . " .wpcf7-submit' placeholder='" . $placeholder . "']");
        } elseif ($tag->has_option('otp_enabled') ) {
            $html .= '<div style="margin-bottom:3%">
			<input class="chatondesk_cf7_otp_btn" type="button" class="button alt" style="width:100%" title="Please Enter a phone number to enable this." value="Click here to verify your Phone"><div id="salert_message" style="background-color: #f7f6f7;padding: 1em 2em 1em 3.5em;display:none;"></div>
			</div>';
    ...
    public function addSmsalertPhoneTag()
    {
        if (class_exists('WPCF7_TagGenerator') ) {
            ...
            $tag_generator->add('billing_phone', __('CHATONDESK PHONE', 'contact-form-7'), array( $this, 'smsalertWpcf7TagGeneratorText' ));
        }
    }
    ...
    <tbody>
        ...
        <?php
        if ('chatondesk_otp_input' === $type ) {
            $field_name = 'chatondesk_customer_validation_otp_token';
        } else {
    ...
    <div class="insert-box">
        ...
        <p class="description mail-tag"><label for="<?php echo esc_attr($args['content'] . '-mailtag'); ?>"><?php echo wp_kses_post(sprintf(__('To use the value input through this field in a mail field, you need to insert the corresponding mail-tag (%s) into the field on the Mail tab.', 'chat-on-desk'), '<strong><span class="mail-tag"></span></strong>')); ?><input type="text" class="mail-tag code hidden" readonly="readonly" id="<?php echo esc_attr($args['content'] . '-mailtag'); ?>" /></label></p>
    </div>
        <?php
    }
    ...
    public function handleCf7ContactForm( $getdata )
    {
        ...
        if (array_key_exists('user_phone', $getdata) && ! \ChatOnDesk\SmsAlertUtility::isBlank($getdata['user_phone']) ) {
            ...
            chatondesk_site_challenge_otp('test', null, null, trim($getdata['user_phone']), 'phone', null, null, true);
        } else {
            wp_send_json(\ChatOnDesk\SmsAlertUtility::_create_json_response(__('Enter a number in the following format : 9xxxxxxxxx', 'chat-on-desk'), \ChatOnDesk\SmsAlertConstants::ERROR_JSON_TYPE));
        }
    }
    ...
    public function validateFormPost( $result, $tag )
    {
        ...
        if (in_array($tag->basetype, array( 'billing_phone', 'chatondesk_otp_input' ), true) ) {
            ...
            if (! Chatondesk::checkPhoneNos($value) ) {
                $result->invalidate($tag, wpcf7_get_message('invalid_no'));
            }
        }

        if (in_array($tag->basetype, array( 'number', 'text', 'tel', 'billing_phone', 'chatondesk_otp_input' ), true) && $name === $this->phone_field_key ) {
            $_SESSION[ $this->form_final_phone_ver ] = $value;
        }

        if (in_array($tag->basetype, array( 'number', 'text', 'billing_phone', 'chatondesk_otp_input' ), true) && 'chatondesk_customer_validation_otp_token' === $name && ! empty($value) ) {
            ...
            if (empty($result->invalid_fields) ) {
                if (! $this->processOTPEntered() ) {
                    $result->invalidate($tag, \ChatOnDesk\SmsAlertUtility::_get_invalid_otp_method());
                } else {
    ...
    public function wpcf7BillingPhoneMessages( $messages )
    {
        ...
        return array_merge(
            ...
            array(
            'invalid_no' => array(
            'description' => __('Invalid number', 'chat-on-desk'),
            'default'     => __('Invalid number', 'chat-on-desk'),
            ),
            )
        );
    }
    ...
    {
        do_action('chatondesk_validate_otp', $_SESSION[ $this->form_session_tag_name ], null);
    }
    ...
    {
        $html  = '<script>jQuery(window).on(\'load\', function(){	jQuery(".chatondesk_cf7_otp_btn,#chatondesk_customer_validation_otp_token").unbind().click(function(o){';
        ...
        $html .= 'target.parents("form").find("#salert_message").show(),jQuery.ajax({url:"' . site_url() . '/?option=chatondesk-cf7-contact",type:"POST",data:{user_phone:e},';
        ...
        $html .= 'target.parents("form").find("#salert_message").css("border-top","3px solid red"),target.parents("form").find("input[name=chatondesk_customer_validation_otp_token]").focus()} ;},';
        $html .= 'error:function(o,e,n){console.log("error"+o)}})});jQuery("[name=chatondesk_customer_validation_otp_token]").on("change",function(){ jQuery(this).find("#salert_message").empty().css("border-top","none")});});</script>';
    ...
    public function addPanelSmsAlert( $form )
    {
        if (wpcf7_admin_has_edit_cap() ) {
            $options = get_option('chatondesk_sms_c7_' . ( method_exists($form, 'id') ? $form->id() : $form->id ));
    ...
    {
        $wpcf7chatondesk_settings = ( ! empty($_POST['wpcf7chatondesk-settings']) ) ? $_POST['wpcf7chatondesk-settings'] : '';
        update_option('chatondesk_sms_c7_' . ( method_exists($form, 'id') ? $form->id() : $form->id ), chatondesk_sanitize_array($wpcf7chatondesk_settings));
    ...
    public function sendsmsC7( $form )
    {
        ...
        if ($send_to_admin ) {
            do_action('cod_send_sms', $admin_number, $admin_message);
        }

        if ($send_to_vistor ) {
            do_action('cod_send_sms', $visitor_number, $visitor_message);
        }

        if (!empty($options['auto_sync']) && 'on' === $options['auto_sync'] ) {
            $obj                   = array();
            $extra_fields          = array();
            $group_name            = $this->getCf7TagSToString($options['chatondesk_group'], $form);
            $obj[0]['person_name'] = $this->getCf7TagSToString($options['chatondesk_name'], $form);
            ...
            if (! empty($form_fields) ) {
                foreach ( $form_fields as $form_field ) {
                    ...
                    if (! empty($field['name']) && '[' . $field['name'] . ']' !== $options['chatondesk_name'] && '[' . $field['name'] . ']' !== $options['visitorNumber'] ) {
                        $extra_fields[ $field['name'] ] = $this->getCf7TagSToString('[' . $field['name'] . ']', $form);
                    }
                }
            }
            $resp = Chatondesk::createContact($obj, $group_name, $extra_fields);
        }
    }
}
new ContactForm7();