 ...
 * Wp-erp class
 */
class Wperp extends \ChatOnDesk\FormInterface
{
    /**
     * Construct function.
     *
    ...
    public static function add_default_setting($defaults = array())
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $defaults['chatondesk_erp_general']['customer_erp_notify_' . $ks]   = 'off';
            $defaults['chatondesk_erp_message']['customer_sms_erp_body_' . $ks] = '';
            $defaults['chatondesk_erp_general']['admin_erp_notify_' . $ks]      = 'off';
            $defaults['chatondesk_erp_message']['admin_sms_erp_body_' . $ks]    = '';
        }
        return $defaults;
    }
    ...
    public static function getCustomerTemplates()
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) {
            $currentVal      = chatondesk_get_option('customer_erp_notify_' . strtolower($vs), 'chatondesk_erp_general', 'on');
            $checkboxNameId  = 'chatondesk_erp_general[customer_erp_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_erp_message[customer_sms_erp_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('customer_sms_erp_body_' . strtolower($vs), 'chatondesk_erp_message', sprintf(__('Hello %1$s, status of your contact with %2$s has been changed to %3$s.%4$sPowered by%5$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[store_name]', $vs, PHP_EOL, PHP_EOL));


            $textBody        = chatondesk_get_option('customer_sms_erp_body_' . strtolower($vs), 'chatondesk_erp_message', $defaultTemplate);
    ...
    public static function getAdminTemplates()
    {
        ...
        $templates           = [];
        foreach ($bookingStatuses as $ks => $vs) {
            $currentVal      = chatondesk_get_option('admin_erp_notify_' . strtolower($vs), 'chatondesk_erp_general', 'on');
            $checkboxNameId  = 'chatondesk_erp_general[admin_erp_notify_' . strtolower($vs) . ']';
            $textareaNameId  = 'chatondesk_erp_message[admin_sms_erp_body_' . strtolower($vs) . ']';
            $defaultTemplate = chatondesk_get_option('admin_sms_erp_body_' . strtolower($vs), 'chatondesk_erp_message', sprintf(__('Hello admin, status of your contact with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', $vs, PHP_EOL, PHP_EOL));


            $textBody = chatondesk_get_option('admin_sms_erp_body_' . strtolower($vs), 'chatondesk_erp_message', $defaultTemplate);
    ...
    public function sendSmsOn($people_id, $args, $people_type)
    {
        ...
        if (empty($args['life_stage'])) {
            $args['life_stage']    = chatondesk_get_option('life_stage', 'erp_settings_erp-crm_contacts', 'lead');
        }
        ...
        $customerMessage = chatondesk_get_option('customer_sms_erp_body_' . $status, 'chatondesk_erp_message', '');


        $customerNotify = chatondesk_get_option('customer_erp_notify_' . $status, 'chatondesk_erp_general', 'on');

        if (($customerNotify === 'on' && $customerMessage !== '')) {
                $buyerMessage = $this->parseSmsBody($args, $customerMessage);
                do_action('cod_send_sms', $phone_no, $buyerMessage);
        }
            // Send msg to admin.
            $adminPhoneNumber = chatondesk_get_option('sms_admin_phone', 'chatondesk_message', '');
        if (empty($adminPhoneNumber) === false) {
            $adminNotify      = chatondesk_get_option('admin_erp_notify_' . $status, 'chatondesk_erp_general', 'on');

            $adminMessage     = chatondesk_get_option('admin_sms_erp_body_' . $status, 'chatondesk_erp_message', '');
