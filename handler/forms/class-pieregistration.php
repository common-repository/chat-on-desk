 ...
 */
class PieRegistrationForm extends \ChatOnDesk\FormInterface
{
     ...
     */
    private $form_session_var = \ChatOnDesk\FormSessionVars::PIE_REG;
    /**
     * Woocommerce registration popup form key
     *
     * @var $form_session_var2 Woocommerce registration popup form key
     */
    private $form_session_var2 = \ChatOnDesk\FormSessionVars::PIE_POPUP;
    ...
    public function saUpdateBillingPhone( $billing_phone, $user_id )
    {
        ...
        if (isset($_SESSION['cod_mobile_pie']) ) {
            $phone = $_SESSION['cod_mobile_pie'];
            unset($_SESSION['cod_mobile_pie']);
    ...
    public function addSmsalertButton( $data )
    {
        ...
        if ($this->popup_enabled ) {
            $data .= do_shortcode('[cod_verify phone_selector="#'.$phone_field.'" submit_selector= ".pie_submit"]').'<script>
        ...
        $data .= '<style>.cod-hide{display:none !important}.pie_register_reg_form .iti{float: left;width: 70%;}</style>';
        return $data;
    }
    ...
    { 
        ...
        if (!$verify) {
            return $errors->add('registration-error-invalid-nonce', __('Sorry, nonce did not verify.', 'chat-on-desk'));
        }
        ...
        $phone_field = $this->getPhoneFieldKey();
        $user_phone = !\ChatOnDesk\SmsAlertUtility::isBlank($phone_field) ? $_POST[$phone_field] : null;
        if ('on' !== chatondesk_get_option('allow_multiple_user', 'chatondesk_general') && ! \ChatOnDesk\SmsAlertUtility::isBlank($user_phone) ) {
            ...
            if (count($getusers) > 0 ) {
                return $errors->add("registration-error-number-exists", __('An account is already registered with this mobile number. Please login.', 'chat-on-desk'));
            }
        }
    ...
    public function processFormFields( $username, $email, $phone, $errors )
    {
        ...
        if (! isset($phone_num) || ! \ChatOnDesk\SmsAlertUtility::validatePhoneNumber($phone_num) ) {
            return $errors->add("billing_phone_error", str_replace('##phone##', $phone_num, $phoneCodLogic->_get_otp_invalid_format_message()));
        }
        chatondesk_site_challenge_otp($username, $email, $errors, $phone_num, 'phone');
    }
    ...
    public function handle_post_verification( $redirect_to, $user_login, $user_email, $password, $phone_number, $extra_data )
    {
        ...
        $_SESSION['cod_mobile_pie']  = $phone_number;
        if (isset($_SESSION[ $this->form_session_var2 ]) ) {
