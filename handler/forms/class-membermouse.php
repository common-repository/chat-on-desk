 ...
 */
class SAMemberMouses extends \ChatOnDesk\FormInterface
{
    /**
     * Construct function.
     *
    ...
    public function handleForm()
    {
        ...
        add_action(
            'chatondesk_followup_sms',  array($this,
            'smsalertSendSms')
        );
    ...
    {
        $customer_notify = chatondesk_get_option('customer_notify', 'chatondesk_mm_renewal', 'on');
        ...
        global $wpdb;
        $table_name           = $wpdb->prefix . 'chatondesk_renewal_reminders';
        ...
        if ('Active' === $status_name && 'on' === $customer_notify && $next_payment_date_dt ) {
            $scheduler_data = get_option('chatondesk_mm_renewal_scheduler');
    ...
    { 
        ...
        if ('on' === $customer_notify && $subscription_details) {
            foreach ( $subscription_details as $subscription ) {
                ...
                do_action('cod_send_sms', $user_phone, $customer_msg);
            }
        }
    } 
    ...
    public static function add_default_setting($defaults = array())
    {
        ...
        foreach ($temp_names as $ks) {
            $defaults['chatondesk_mm_general']['customer_mm_notify_' . $ks]   = 'off';
            $defaults['chatondesk_mm_message']['customer_sms_mm_body_' . $ks] = '';
            $defaults['chatondesk_mm_general']['admin_mm_notify_' . $ks]      = 'off';
            $defaults['chatondesk_mm_message']['admin_sms_mm_body_' . $ks]    = '';
        }
        $defaults['chatondesk_mm_renewal']['customer_notify']                  = 'off';
        $defaults['chatondesk_mm_renewal_scheduler']['cron'][0]['frequency']  = '1';
        $defaults['chatondesk_mm_renewal_scheduler']['cron'][0]['message']    = '';
        $defaults['chatondesk_mm_renewal_scheduler']['cron'][1]['frequency']  = '2';
        $defaults['chatondesk_mm_renewal_scheduler']['cron'][1]['message']    = '';
        return $defaults;
    }
    ...
    {
        $current_val      = chatondesk_get_option('customer_notify', 'chatondesk_mm_renewal', 'on');
        $checkbox_name_id = 'chatondesk_mm_renewal[customer_notify]';
        ...
        if (empty($scheduler_data) ) {
            ...
            'frequency' => '1',
            'message'   => sprintf(__('Hello %1$s, your subscription %2$s with %3$s is due for renewal on %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '#[member_id]', '[store_name]', '[next_payment_date]', PHP_EOL, PHP_EOL),
        ...
        foreach ( $scheduler_data['cron'] as $key => $data ) {
            ...
            $text_area_name_id = 'chatondesk_mm_renewal_scheduler[cron][' . $count . '][message]';
            $select_name_id    = 'chatondesk_mm_renewal_scheduler[cron][' . $count . '][frequency]';
    ...
    public static function getCustomerTemplates()
    {
        $templates           = [];
         ...
         /*  When member add */
        $currentVal         = chatondesk_get_option('customer_mm_notify_member_added', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[customer_mm_notify_member_added]';
            $textareaNameId  = 'chatondesk_mm_message[customer_sms_mm_body_member_added]';
            $defaultTemplate = chatondesk_get_option('customer_sms_mm_body_membership_status', 'chatondesk_mm_message', sprintf(__('Hello %1$s, status of your membership #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[member_id]', '[store_name]', '[status_name]', PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_mm_body_member_added', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('customer_mm_notify_mm_status', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[customer_mm_notify_mm_status]';
            $textareaNameId  = 'chatondesk_mm_message[customer_sms_mm_body_mm_status]';
            $defaultTemplate = chatondesk_get_option('customer_sms_mm_body_mm_status', 'chatondesk_mm_message', sprintf(__('Hello %1$s, status of your membership #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[member_id]', '[store_name]', '[status_name]', PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_mm_body_mm_status', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('customer_mm_notify_membership_level', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[customer_mm_notify_membership_level]';
            $textareaNameId  = 'chatondesk_mm_message[customer_sms_mm_body_membership_level]';
            $defaultTemplate = chatondesk_get_option('customer_sms_mm_body_membership_level', 'chatondesk_mm_message', sprintf(__('Hello %1$s, status of your membership #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[member_id]', '[store_name]', '[membership_level_name]', PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_mm_body_membership_level', 'chatondesk_mm_message', $defaultTemplate);
        ...
        /*  When new bondle add*/
        $currentVal      = chatondesk_get_option('customer_mm_notify_bundle_status', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[customer_mm_notify_add_bundle]';
            $textareaNameId  = 'chatondesk_mm_message[customer_sms_mm_body_add_bundle]';
            $defaultTemplate = chatondesk_get_option('customer_sms_mm_body_bundle_status', 'chatondesk_mm_message', sprintf(__('Hello %1$s, status of your bundle #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[bundle_id]', '[store_name]', '[bundle_name]', PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_mm_body_add_bundle', 'chatondesk_mm_message', $defaultTemplate);
        ...
        /*  When membership level update*/
        $currentVal      = chatondesk_get_option('customer_mm_notify_bundle_status', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[customer_mm_notify_bundle_status]';
            $textareaNameId  = 'chatondesk_mm_message[customer_sms_mm_body_bundle_status]';
            $defaultTemplate = chatondesk_get_option('customer_sms_mm_body_bundle_status', 'chatondesk_mm_message', sprintf(__('Hello %1$s, status of your bundle #%2$s with %3$s has been changed to %4$s.%5$sPowered by%6$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[bundle_id]', '[store_name]', '[bundle_status_name]', PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_mm_body_bundle_status', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('customer_mm_notify_mm_payment', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[customer_mm_notify_mm_payment]';
            $textareaNameId  = 'chatondesk_mm_message[customer_sms_mm_body_mm_payment]';
            $defaultTemplate = chatondesk_get_option('customer_sms_mm_body_mm_payment', 'chatondesk_mm_message', sprintf(__('Hello %1$s, payment status for your order #%2$s with %3$s has been changed to completed.%4$sPowered by%5$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[order_transaction_id]', '[store_name]', PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_mm_body_mm_payment', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('customer_mm_notify_mm_payment_refund', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[customer_mm_notify_mm_payment_refund]';
            $textareaNameId  = 'chatondesk_mm_message[customer_sms_mm_body_mm_payment_refund]';
            $defaultTemplate = chatondesk_get_option('customer_sms_mm_body_mm_payment_refund', 'chatondesk_mm_message', sprintf(__('Hello %1$s, payment status for your order #%2$s with %3$s has been changed to refunded.%4$sPowered by%5$swww.chatondesk.com', 'chat-on-desk'), '[first_name]', '[order_transaction_id]', '[store_name]', PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('customer_sms_mm_body_mm_payment_refund', 'chatondesk_mm_message', $defaultTemplate);
    ...
    {
        $templates           = [];
        $currentVal      = chatondesk_get_option('admin_mm_notify_member_added', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[admin_mm_notify_member_added]';
            $textareaNameId  = 'chatondesk_mm_message[admin_sms_mm_body_member_added]';
            $defaultTemplate = chatondesk_get_option('admin_sms_mm_body_member_added', 'chatondesk_mm_message', sprintf(__('Hello admin, status of your membership with %1$s has been changed to %2$s. %3$sPowered by%4$swww.chatondesk.com', 'chat-on-desk'), '[store_name]', '[status_name]', PHP_EOL, PHP_EOL));

            $textBody = chatondesk_get_option('admin_sms_mm_body_member_added', 'chatondesk_mm_message', $defaultTemplate);
            ...
            $currentVal      = chatondesk_get_option('admin_mm_notify_mm_status', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[admin_mm_notify_mm_status]';
            $textareaNameId  = 'chatondesk_mm_message[admin_sms_mm_body_mm_status]';
            $defaultTemplate = chatondesk_get_option('admin_sms_mm_body_mm_status', 'chatondesk_mm_message', sprintf(__('%1$s status of membership has been changed to %2$s.', 'chat-on-desk'), '[store_name]:', '[status_name]'));

            $textBody = chatondesk_get_option('admin_sms_mm_body_mm_status', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('admin_mm_notify_membership_level', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[admin_mm_notify_membership_level]';
            $textareaNameId  = 'chatondesk_mm_message[admin_sms_mm_body_membership_level]';
            $defaultTemplate = chatondesk_get_option('admin_sms_mm_body_membership_level', 'chatondesk_mm_message', sprintf(__('%1$s status of membership has been changed to %2$s.', 'chat-on-desk'), '[store_name]:', '[membership_level_name]'));

            $textBody = chatondesk_get_option('admin_sms_mm_body_membership_level', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('admin_mm_notify_add_bundle', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[admin_mm_notify_add_bundle]';
            $textareaNameId  = 'chatondesk_mm_message[admin_sms_mm_body_add_bundle]';
            $defaultTemplate = chatondesk_get_option('admin_sms_mm_body_bundle_status', 'chatondesk_mm_message', sprintf(__('%1$s status of bundle has been changed to %2$s.', 'chat-on-desk'), '[store_name]:', '[bundle_status_name]'));

            $textBody = chatondesk_get_option('admin_sms_mm_body_add_bundle', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('admin_mm_notify_bundle_status', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[admin_mm_notify_bundle_status]';
            $textareaNameId  = 'chatondesk_mm_message[admin_sms_mm_body_bundle_status]';
            $defaultTemplate = chatondesk_get_option('admin_sms_mm_body_bundle_status', 'chatondesk_mm_message', sprintf(__('%1$s status of bundle has been changed to %2$s.', 'chat-on-desk'), '[store_name]:', '[bundle_status_name]'));

            $textBody = chatondesk_get_option('admin_sms_mm_body_bundle_status', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('admin_mm_notify_mm_payment', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[admin_mm_notify_mm_payment]';
            $textareaNameId  = 'chatondesk_mm_message[admin_sms_mm_body_mm_payment]';
            $defaultTemplate = chatondesk_get_option('admin_sms_mm_body_mm_payment', 'chatondesk_mm_message', sprintf(__('%1$s status of membership has been changed to %2$s.', 'chat-on-desk'), '[store_name]:', '[status_name]'));

            $textBody = chatondesk_get_option('admin_sms_mm_body_mm_payment', 'chatondesk_mm_message', $defaultTemplate);
        ...
        $currentVal      = chatondesk_get_option('admin_mm_notify_mm_payment_refund', 'chatondesk_mm_general', 'on');
            $checkboxNameId  = 'chatondesk_mm_general[admin_mm_notify_mm_payment_refund]';
            $textareaNameId  = 'chatondesk_mm_message[admin_sms_mm_body_mm_payment_refund]';
            $defaultTemplate = chatondesk_get_option('admin_sms_mm_body_mm_payment_refund', 'chatondesk_mm_message', sprintf(__('%1$s status of membership has been changed to %2$s.', 'chat-on-desk'), '[store_name]:', '[status_name]'));

            $textBody = chatondesk_get_option('admin_sms_mm_body_mm_payment_refund', 'chatondesk_mm_message', $defaultTemplate);
    ...
    public function sendSmsOn($data, $status,$entity)
    {
        ...
        $customerMessage   = chatondesk_get_option('customer_sms_mm_body_'.$entity, 'chatondesk_mm_message', '');
        
        $customerNotify    = chatondesk_get_option('customer_mm_notify_'.$entity, 'chatondesk_mm_general', 'on');
        ...
        if (empty($adminPhoneNumber) === false) {
            $adminNotify        = chatondesk_get_option('admin_mm_notify_'.$entity, 'chatondesk_mm_general', 'on');
            $adminMessage       = chatondesk_get_option('admin_sms_mm_body_'.$entity, 'chatondesk_mm_message', '');
