...
class Plugin_Feedback
{
    ...
    public function __construct()
    {
        ...
        add_filter('chatondesk_deactivation_form_fields', array( $this, 'addDeactivationFormFields' ));

        // Ajax to send data.
        add_action('wp_ajax_cod_send_onboarding_data', array( $this, 'sendOnboardingData' ));
        add_action('wp_ajax_nopriv_cod_send_onboarding_data', array( $this, 'sendOnboardingData' ));
    ...
    public function enqueueStyles()
    {
        if ($this->isValidPageScreen() ) {
            ...
            wp_enqueue_style('admin-cod-feedback-style', plugins_url('../css/feedback-admin.css', __FILE__), array(), \ChatOnDesk\SmsAlertConstants::SA_VERSION);
        }
    }
    ...
    public function enqueueScripts()
    {
        ...
        if ($this->isValidPageScreen() ) {
            ...
            wp_enqueue_script('admin-cod-feedback-scripts', plugins_url('../js/feedback-admin.js', __FILE__), array( 'jquery' ), \ChatOnDesk\SmsAlertConstants::SA_VERSION, true);

            global $pagenow;
            $current_slug = ! empty(explode('/', plugin_basename(__FILE__))) ? explode('/', plugin_basename(__FILE__))[0] : '';

            wp_localize_script(
                'admin-cod-feedback-scripts',
                'codf',
                array(
                'ajaxurl'                => admin_url('admin-ajax.php'),
                'auth_nonce'             => wp_create_nonce('codf_onboarding_nonce'),
    ...
    public function renderFieldHtml( $attr = array(), $base_class = 'on-boarding' )
    {
        ...
        if ('hidden' !== $type ) : ?>
            <div class ="codf-customer-data-form-single-field">
            <?php
        endif;
        ...
        case 'radio':
            ...
            if (! empty($options) && is_array($options) ) :
                ...
                foreach ( $options as $option_value => $option_label ) :
                    ?>
                        <div class="codf-<?php echo esc_html($base_class); ?>-radio-wrapper">
    ...
    public function sendOnboardingData()
    {
        ...
        check_ajax_referer('codf_onboarding_nonce', 'nonce');
        ...
        $param['name'] =  chatondesk_get_option('chatondesk_name', 'chatondesk_gateway', '');

        $url = 'https://apps.smsalert.co.in/apps/wordpress/webhook.php?topic=app/uninstalled';
        $result = Chatondesk::callAPI($url, $param);
    ...
    /**
     * Add codf additional validation.
     *
     * @param $result result 
     *
     * @return array
    ...
    public function addSmsfDeactivationScreens( $valid_screens = array() )
    {
        ...
        if (is_array($valid_screens) ) {
            ...
            // Push your screen here.
            array_push($valid_screens, 'chat-on-desk');
        }
