...
class Share_Cart
{
    ...
    public function __construct()
    {
        ...
        if ($user_authorize->is_user_authorised()) {
            $share_cart_enable = chatondesk_get_option('customer_notify', 'chatondesk_share_cart_general');
            ...
            if ('on' === $share_cart_enable ) {
                ...
                add_shortcode('cod_sharecart', array( $this, 'addShareCartButton' ), 100);
            }

            add_action('wp_loaded', array( $this, 'restoreSharCart' ));
            add_action('wp_ajax_cod_check_cart_data', array( $this, 'checkCartIsEmpty' ));
            add_action('wp_ajax_nopriv_cod_check_cart_data', array( $this, 'checkCartIsEmpty' ));

            add_action('wp_ajax_cod_save_cart_data', array( $this, 'saveShareCartData' ));
            add_action('wp_ajax_nopriv_cod_save_cart_data', array( $this, 'saveShareCartData' ));
        }
    }
    ...
    {
        wp_enqueue_script('share_cart_cod_admin_script', COD_MOV_URL . 'js/share_cart_front_script.js', false, \ChatOnDesk\SmsAlertConstants::SA_VERSION);
        wp_localize_script(
            'share_cart_cod_admin_script',
            'ajax_url',
            array(
    ...
    {
		$defaults['chatondesk_share_cart_general']['customer_notify'] = 'off';
        $defaults['chatondesk_share_cart_general']['share_btnpos']    = 'after_cart_table';
        $defaults['chatondesk_share_cart_general']['share_btntext']   = 'Share Cart';
        $defaults['chatondesk_share_cart_message']['customer_notify'] = '';
        return $defaults;
    }

    /**
     * Get Chat On Desk share cart templates.
     *
     * @return array
     */
    public function getSmsAlertShareCartTemplates()
    {
        $current_val      = chatondesk_get_option(
            'customer_notify',
            'chatondesk_share_cart_general
		',
            'on'
        );
        $checkbox_name_id = 'chatondesk_share_cart_general[customer_notify]';

        $templates = array();

        $textarea_name_id = 'chatondesk_share_cart_message[customer_notify]';
        $text_body        = chatondesk_get_option(
            'customer_notify',
            'chatondesk_share_cart_message',
    ...
    {
		$share_btnpos = '';
        if (! empty(chatondesk_get_option('share_btnpos', 'chatondesk_share_cart_general')) ) {
            $share_btnpos = chatondesk_get_option('share_btnpos', 'chatondesk_share_cart_general');
        }
    ...
    {
		echo do_shortcode('[cod_sharecart]');
    }
     ...
     * @return array
     */
    function addShareCartButton()
    {
        global $woocommerce;
        $cart_total = isset(WC()->cart->total)?WC()->cart->total:'';
        if (!empty($cart_total)) {
            add_action('wp_footer', array( $this, 'shareCartPopupDiv' ));    
            $btn_text = chatondesk_get_option('share_btntext', 'chatondesk_share_cart_general');
            return '<button class="button button-primary" id="chatondesk_share_cart"><span class="button__text">' . esc_html__($btn_text) . '</span></button>';
    ...
    function shareCartPopupDiv()
    {
        ...
        $modal_style = chatondesk_get_option('modal_style', 'chatondesk_general', 'center');
		...
		if ( is_plugin_active('elementor/elementor.php') && !empty($post)) {  
			...
			$content = \CodPopup::getShareCartStyle();
		
		}
        ?>
        <div id="chatondesk_sharecart_popup" class="smsalert_chatondesk_popup_class chatondeskModal <?php echo esc_attr($modal_style); ?>" data-modal-close="<?php echo esc_attr(substr($modal_style, 0, -2)); ?>">
        <?php 
		 echo $content; 
        ?>  
        </div>
    ...
    {
        $verify = check_ajax_referer('chatondesk_wp_sharecart_nonce', 'chatondesk_sharecart_nonce', false);
        if (!$verify) {
            echo 'Sorry, nonce did not verify.';
            die();
        }
        if (isset($_REQUEST['sc_cod_umobile']) ) {
            global $phoneCodLogic;
            
            $invalid_fmob= str_replace('##phone##', $_REQUEST['sc_cod_fmobile'], $phoneCodLogic->_get_otp_invalid_format_message());
            
            $invalid_scmob= str_replace('##phone##', $_REQUEST['sc_cod_umobile'], $phoneCodLogic->_get_otp_invalid_format_message());
            
            $_REQUEST['sc_cod_fmobile'] = Chatondesk::checkPhoneNos($_REQUEST['sc_cod_fmobile']);
            
            $_REQUEST['sc_cod_umobile'] = Chatondesk::checkPhoneNos($_REQUEST['sc_cod_umobile']);
            
            if (empty($_REQUEST['sc_cod_umobile'])) {
                echo $invalid_scmob;die();
            }
            if (empty($_REQUEST['sc_cod_fmobile'])) {
            ...
            $name  = sanitize_text_field($_REQUEST['sc_cod_uname']);
            $phone = sanitize_text_field($_REQUEST['sc_cod_umobile']);
            ...
            if ($_REQUEST['sc_cod_fmobile'] ) {
                ...
                $data['friend_name']  = sanitize_text_field($_REQUEST['sc_cod_fname']);
                $data['friend_phone'] = sanitize_text_field($_REQUEST['sc_cod_fmobile']);
                $data['your_phone']   = sanitize_text_field($_REQUEST['sc_cod_umobile']);
                $data['your_name']    = sanitize_text_field($_REQUEST['sc_cod_uname']);
                $phone                = sanitize_text_field($_REQUEST['sc_cod_fmobile']);
                $message              = chatondesk_get_option('customer_notify', 'chatondesk_share_cart_message');
                do_action('cod_send_sms', $phone, $this->parseSmsBody($data, $message));
            }
