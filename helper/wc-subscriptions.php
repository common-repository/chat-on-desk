...
class WCSubscription
{
    ...
    public function __construct()
    {
        ...
        add_action('chatondesk_followup_sms', array( $this, 'smsalertSendSms' ));
    }
    ...
    public function add_default_setting( $defaults = array() )
    {
        ...
        foreach ( $statuses as $ks => $order_status ) {
            ...
            $defaults['chatondesk_wcs_general'][ 'admin_subs_' . $vs . '_msg' ]          = 'off';
            $defaults['chatondesk_wcs_general'][ 'cust_subs_' . $vs . '_msg' ]           = 'off';
            $defaults['chatondesk_wcs_message'][ 'sms_body_admin_subs_' . $vs . '_msg' ] = '';
            $defaults['chatondesk_wcs_message'][ 'sms_body_cust_subs_' . $vs . '_msg' ]  = '';

        }
        $defaults['chatondesk_wc_renewal']['customer_notify']                = 'off';
        $defaults['chatondesk_wc_renewal']['followup_sms_time']              = '10:00';
        $defaults['chatondesk_wc_renewal_scheduler']['cron'][0]['frequency'] = '1';
        $defaults['chatondesk_wc_renewal_scheduler']['cron'][0]['message']   = '';
        $defaults['chatondesk_wc_renewal_scheduler']['cron'][1]['frequency'] = '2';
        $defaults['chatondesk_wc_renewal_scheduler']['cron'][1]['message']   = '';
        return $defaults;
    }
    ...
    public static function getCustomerTemplates()
    {
        ...
        foreach ( $statuses as $ks  => $order_status ) {
            ...
            $current_val = chatondesk_get_option('cust_subs_' . $vs . '_msg', 'chatondesk_wcs_general', 'on');

            $check_box_name_id = 'chatondesk_wcs_general[cust_subs_' . $vs . '_msg]';
            $text_area_name_id = 'chatondesk_wcs_message[sms_body_cust_subs_' . $vs . '_msg]';
            ...
            $text_body        = chatondesk_get_option('sms_body_cust_subs_' . $vs . '_msg', 'chatondesk_wcs_message', ( ( '' !== $default_template ) ? $default_template : '' ));
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ( $statuses as $ks  => $order_status ) {
            ...
            $current_val = chatondesk_get_option('admin_subs_' . $vs . '_msg', 'chatondesk_wcs_general', 'on');

            $check_box_name_id = 'chatondesk_wcs_general[admin_subs_' . $vs . '_msg]';
            $text_area_name_id = 'chatondesk_wcs_message[sms_body_admin_subs_' . $vs . '_msg]';
            ...
            $text_body        = chatondesk_get_option('sms_body_admin_subs_' . $vs . '_msg', 'chatondesk_wcs_message', ( ( '' !== $default_template ) ? $default_template : '' ));
    ...
    {
        $current_val      = chatondesk_get_option('customer_notify', 'chatondesk_wc_renewal', 'on');
        $checkbox_name_id = 'chatondesk_wc_renewal[customer_notify]';

        $scheduler_data = get_option('chatondesk_wc_renewal_scheduler');
        $templates      = array();
        $count          = 0;
        ...
        foreach ( $scheduler_data['cron'] as $key => $data ) {
            ...
            $text_area_name_id = 'chatondesk_wc_renewal_scheduler[cron][' . $count . '][message]';
            $select_name_id    = 'chatondesk_wc_renewal_scheduler[cron][' . $count . '][frequency]';
    ...
    public function smsalertSendMsgSubsStatusChange( $subscription )
    {
        ...
        if ($order_id > 0) {
            ...
            $admin_msg                  = chatondesk_get_option('sms_body_admin_subs_' . $subs_status . '_msg', 'chatondesk_wcs_message', '');
            ...
            $chatondesk_notification_subs_status_change_admin_msg = chatondesk_get_option('admin_subs_' . $subs_status . '_msg', 'chatondesk_wcs_general', 'on');

            if ('on' === $chatondesk_notification_subs_status_change_admin_msg && '' !== $admin_message ) {
                do_action('cod_send_sms', $sms_admin_phone, $admin_message);
            }

            $customer_msg              = chatondesk_get_option('sms_body_cust_subs_' . $subs_status . '_msg', 'chatondesk_wcs_message', '');
            ...
            $chatondesk_notification_subs_status_change_cust_msg = chatondesk_get_option('cust_subs_' . $subs_status . '_msg', 'chatondesk_wcs_general', 'on');
            if ('on' === $chatondesk_notification_subs_status_change_cust_msg && '' !== $customer_msg ) {
                do_action('cod_send_sms', $cust_no, $customer_msg);
            }
        }
    }
    ...
    public function smsalertSendMsgSubsRenewal( $subscription, $order )
    {
        ...
        $admin_msg                  = chatondesk_get_option('sms_body_admin_subs_renewal_msg', 'chatondesk_wcs_message', '');
        $admin_msg                  = $this->parseSmsBody($subscription, $admin_msg);
        $admin_sms_data['number']   = $sms_admin_phone;
        $admin_sms_data['sms_body'] = $admin_msg;
        $admin_sms_data             = WooCommerceCheckOutForm::pharseSmsBody($admin_sms_data, $order_id);
        ...
        $chatondesk_notification_subs_renewal_admin_msg = chatondesk_get_option('admin_subs_renewal_msg', 'chatondesk_wcs_general', 'on');

        if ('on' === $chatondesk_notification_subs_renewal_admin_msg && '' !== $admin_message ) {
            do_action('cod_send_sms', $sms_admin_phone, $admin_message);
        }

        $customer_msg              = chatondesk_get_option('sms_body_cust_subs_renewal_msg', 'chatondesk_wcs_message', '');
        $customer_msg              = $this->parseSmsBody($subscription, $customer_msg);
        $cust_sms_data['number']   = $cust_no;
        $cust_sms_data['sms_body'] = $customer_msg;
        $cust_sms_data             = WooCommerceCheckOutForm::pharseSmsBody($cust_sms_data, $order_id);
        ...
        $chatondesk_notification_subs_renewal_cust_msg = chatondesk_get_option('cust_subs_renewal_msg', 'chatondesk_wcs_general', 'on');
        if ('on' === $chatondesk_notification_subs_renewal_cust_msg && '' !== $customer_msg ) {
            do_action('cod_send_sms', $cust_no, $customer_msg);
        }
    }
    ...
    public function smsalertSendMsgSubsCreated( $subscription, $order, $recurring_cart )
    {
        ...
        $admin_msg                  = chatondesk_get_option('sms_body_admin_subs_create_msg', 'chatondesk_wcs_message', '');
        $admin_msg                  = $this->parseSmsBody($subscription, $admin_msg);
        $admin_sms_data['number']   = $sms_admin_phone;
        $admin_sms_data['sms_body'] = $admin_msg;
        $admin_sms_data             = WooCommerceCheckOutForm::pharseSmsBody($admin_sms_data, $order_id);
        ...
        $chatondesk_notification_subs_create_admin_msg = chatondesk_get_option('admin_subs_create_msg', 'chatondesk_wcs_general', 'on');

        if ('on' === $chatondesk_notification_subs_create_admin_msg && '' !== $admin_message ) {
            do_action('cod_send_sms', $sms_admin_phone, $admin_message);
        }

        $customer_msg              = chatondesk_get_option('sms_body_cust_subs_create_msg', 'chatondesk_wcs_message', '');
        $customer_msg              = $this->parseSmsBody($subscription, $customer_msg);
        $cust_sms_data['number']   = $cust_no;
        $cust_sms_data['sms_body'] = $customer_msg;
        $cust_sms_data             = WooCommerceCheckOutForm::pharseSmsBody($cust_sms_data, $order_id);
        ...
        $chatondesk_notification_subs_create_cust_msg = chatondesk_get_option('cust_subs_create_msg', 'chatondesk_wcs_general', 'on');
        if ('on' === $chatondesk_notification_subs_create_cust_msg && '' !== $customer_msg ) {
            do_action('cod_send_sms', $cust_no, $customer_msg);
        }
    }
    ...
    {
        $customer_notify = chatondesk_get_option('customer_notify', 'chatondesk_wc_renewal', 'on');
