...
class SA_Abandoned_Cart
{
    ...
    public function __construct()
    {
        ...
        $this->version     = 'chat-on-desk';

        $this->loadDependencies();
        if ('on' === chatondesk_get_option('customer_notify', 'chatondesk_abandoned_cart') ) {
    ...
    { 
        echo wp_nonce_field('chatondesk_wp_abcart_nonce', 'chatondesk_abcart_nonce', true, false);
    }
    ...
    private function defineAdminHooks()
    {
        ...
        $this->loader->add_action('ab_cart_cod_notification_sendsms_hook', $plugin_admin, 'sendSms'); // Hooks into WordPress cron event to launch function for sending out SMS
    ...
    private function definePublicHooks()
    {
        ...
        $this->loader->add_action('wp_ajax_nopriv_cod_save_data', $plugin_public, 'saveUserData'); // Handles data saving using Ajax after any changes made by the user on the Phone field in Checkout form
        $this->loader->add_action('wp_ajax_cod_save_data', $plugin_public, 'saveUserData'); // Handles data saving using Ajax after any changes made by the user on the Mobile field for Logged in users
    ...
    {
        $defaults['chatondesk_abandoned_cart']['notification_frequency']         = '10';
        $defaults['chatondesk_abandoned_cart']['cart_exit_intent_status']        = '';
        $defaults['chatondesk_abandoned_cart']['enable_quiet_hours'] = '';
        $defaults['chatondesk_abandoned_cart']['from_quiet_hours'] = '22:00';
        $defaults['chatondesk_abandoned_cart']['to_quiet_hours'] = '08:00';
        $defaults['chatondesk_abandoned_cart']['customer_notify']                = 'off';
        $defaults['chatondesk_abandoned_cart_scheduler']['cron'][0]['frequency'] = '60';
        $defaults['chatondesk_abandoned_cart_scheduler']['cron'][0]['message']   = '';
        $defaults['chatondesk_abandoned_cart_scheduler']['cron'][1]['frequency'] = '120';
        $defaults['chatondesk_abandoned_cart_scheduler']['cron'][1]['message']   = '';
    ...
    /**
     * Get Chat On Desk cart templates.
     *
     * @return array
     */
    public function getSmsAlertCartTemplates()
    {
        $current_val      = chatondesk_get_option('customer_notify', 'chatondesk_abandoned_cart', 'on');
        $checkbox_name_id = 'chatondesk_abandoned_cart[customer_notify]';

        $scheduler_data = get_option('chatondesk_abandoned_cart_scheduler');
        $templates      = array();
        $count          = 0;
        ...
        foreach ( $scheduler_data['cron'] as $key => $data ) {
            $textarea_name_id = 'chatondesk_abandoned_cart_scheduler[cron][' . $count . '][message]';
            $selectNameId     = 'chatondesk_abandoned_cart_scheduler[cron][' . $count . '][frequency]';
...
class SA_Cart_Admin
{
    ...
    public static function display_page()
    {
        ...
        ?>
        <div class="wrap">
            <h1>Abandoned Cart <a href="admin.php?page=cod-ab-cart-reports" class="button action">View Reports</a></h1>
            <h2 id="heading-for-admin-notice-dislay"></h2>

        <?php
        if ('admin.php' === $pagenow && 'cod-ab-cart' === $_GET['page'] ) {
            if ($deleted ) {
                ?>
            ...
            if (0 === self::abandonedCartCount() ) { // If no abandoned carts, then output this note
                ?>
                <p>
                <?php esc_html_e('Looks like you do not have any saved Abandoned carts yet.<br/>But do not worry, as soon as someone fills the <strong>Phone number</strong> fields of your WooCommerce Checkout form and abandons the cart, it will automatically appear here.', 'chat-on-desk'); ?>
                </p>
            <?php } else { ?>
                <form method="GET">
    ...
    public static function initialiseStartEndDate()
    {
     ...
     $chatondesk_date_range = isset( $_GET['chatondesk_date_range'] ) ? sanitize_text_field( wp_unslash( $_GET['chatondesk_date_range'] ) ) : 'this_month'; //phpcs:ignore

     $current_time  = current_time( 'timestamp' ); // phpcs:ignore
     $current_month = date( 'n' ); //phpcs:ignore
     $current_year  = date( 'Y' ); //phpcs:ignore

        switch ( $chatondesk_date_range ) {
       ...
       self::$end_timestamp   = mktime( 23, 59, 59, $current_month - 1, date( 't' ) ); //phpcs:ignore
        ...
        case 'custom':
            $user_start = isset($_GET['cod_start_date']) ? sanitize_text_field(wp_unslash($_GET['cod_start_date'])) : ''; // phpcs:ignore WordPress.Security.NonceVerification
            $user_end   = isset($_GET['cod_end_date']) ? sanitize_text_field(wp_unslash($_GET['cod_end_date'])) : ''; // phpcs:ignore WordPress.Security.NonceVerification
    ...
    public static function getAbandonedData()
    {
     ...
     $selected_data_range = isset( $_GET['chatondesk_date_range'] ) ? sanitize_text_field( wp_unslash( $_GET['chatondesk_date_range'] ) ) : 'this_month'; //phpcs:ignore
    ...
    {
        if ('cod-ab-cart-reports' === $_GET['page'] ) {
            wp_enqueue_script(
                'd3_js',
                COD_MOV_URL . 'js/d3.v3.min.js',
                '',
                \ChatOnDesk\SmsAlertConstants::SA_VERSION,
                false
            );
            wp_register_script('reports_js', COD_MOV_URL . 'js/ab-cart-graph.js', '', \ChatOnDesk\SmsAlertConstants::SA_VERSION, false);

            $cod_duration_range_select = array(
            'this_month'   => __('This Month', 'chat-on-desk'),
            'last_month'   => __('Last Month', 'chat-on-desk'),
            'this_quarter' => __('This Quarter', 'chat-on-desk'),
            'last_quarter' => __('Last Quarter', 'chat-on-desk'),
            'this_year'    => __('This Year', 'chat-on-desk'),
            'last_year'    => __('Last Year', 'chat-on-desk'),
            'custom'       => __('Custom', 'chat-on-desk'),
            );
            $cod_duration_range        = isset($_GET['chatondesk_date_range']) ? sanitize_text_field(wp_unslash($_GET['chatondesk_date_range'])) : 'this_month';

            $start_date              = isset($_GET['cod_start_date']) ? sanitize_text_field(wp_unslash($_GET['cod_start_date'])) : '';
            $end_date                = isset($_GET['cod_end_date']) ? sanitize_text_field(wp_unslash($_GET['cod_end_date'])) : '';
            $start_end_date_div_show = ( ! isset($_GET['chatondesk_date_range']) || 'custom' !== $_GET['chatondesk_date_range'] ) ? 'none' : 'block';
            ?>
        <div class="wrap">
            <h1>Abandoned Cart Reports <a href="admin.php?page=cod-ab-cart" class="button action">View List</a></h1>
            <h2 id="heading-for-admin-notice-dislay"></h2>
                <form method="GET">
                    <input type="hidden" name="page" value="<?php echo esc_attr($_REQUEST['page']); ?>"/>
                ...
                <div class = "filter_date_drop_down" id = "filter_date_drop_down" >
                    <label class="date_time_filter_label" for="date_time_filter_label">
                        <strong>
            <?php esc_html_e('Select date range:', 'chat-on-desk'); ?>
                        </strong>
                    </label>

                    <select id=chatondesk_date_range name="chatondesk_date_range">
            <?php
            foreach ( $cod_duration_range_select as $key => $value ) {
                $sel = '';
                if ($key == $cod_duration_range ) {
                    $sel = 'selected';
                }
             echo sprintf( "<option value='%s' %s>%s</option>", esc_attr( $key ), esc_attr( $sel ), esc_attr( __( $value, 'chat-on-desk' ) ) ); //phpcs:ignore
            }
            ?>
                    </select>
                    <div class = "cod_start_end_date_div" id = "cod_start_end_date_div" style="display: <?php echo esc_attr($start_end_date_div_show); ?>"  >
                        <input type="date" id="cod_start_date" name="cod_start_date" value="<?php echo esc_attr($start_date); ?>" placeholder="yyyy-mm-dd"/>
                        <input type="date" id="cod_end_date" name="cod_end_date" value="<?php echo esc_attr($end_date); ?>" placeholder="yyyy-mm-dd"/>
                    </div>
                    <div id="cod_submit_button" class="cod_submit_button">
                        <button type="submit" class="button-primary" id="cod_search" value="go"><?php esc_html_e('Go', 'chat-on-desk'); ?></button>
                        <a href="admin.php?page=cod-ab-cart-reports" class="button-secondary">Reset</a>
                    </div>
        ...
        <div class="clear"></div>
        <div id="cod_counter_container">
        <div class="counter_widget">
        <div id="cod_total_abandoned_amount" class="cod_counter_container">
        <div class="cod_widget_title_container">
        <span class="cod_counter_title">Abandoned Amount</span>
        </div>
        <div class="cod_counter_body">
        ...
        <div class="counter_widget">
                            <div id="cod_total_recovered_count" class="cod_counter_container">
        <div class="cod_widget_title_container">
        <span class="cod_counter_title">Abandoned Carts</span>
        </div>
        <div class="cod_counter_body">
            <?php echo $result['abandoned_count']; ?>        </div>
                ...
                <div class="counter_widget">
                            <div id="cod_total_recovered_amount" class="cod_counter_container">
        <div class="cod_widget_title_container">
        <span class="cod_counter_title">Recovered Amount</span>
        </div>
        <div class="cod_counter_body">
        <span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol"><?php echo $result['currency']; ?></span><?php echo ' ' . $result['recovered_amount']; ?></bdi></span>        </div>
                    ...
                    <div class="counter_widget">
                <div id="cod_total_abandoned_count" class="cod_counter_container">
        <div class="cod_widget_title_container">
        <span class="cod_counter_title">Recovered Carts</span>
        </div>
        <div class="cod_counter_body">
            <?php echo $result['recovered_count']; ?></div>
                ...
                <div class="counter_widget count_widget_right">
                            <div id="cod_total_recovery_rate" class="cod_counter_container">
        <div class="cod_widget_title_container">
        <span class="cod_counter_title">SMS Sent</span>
        </div>
        <div class="cod_counter_body">
    ...
    function displayWpCronWarnings()
    {
        ...
        if ('admin.php' === $pagenow && 'chat-on-desk' === sanitize_text_field($_GET['page']) ) {
            ...
            if (wp_next_scheduled('ab_cart_cod_notification_sendsms_hook') === false ) { // If we havent scheduled msg notifications and notifications have not been disabled
                $missing_hooks[] = 'ab_cart_cod_notification_sendsms_hook';
            }
    ...
    function sendSms()
    {
        $notification_enabled = chatondesk_get_option('customer_notify', 'chatondesk_abandoned_cart', 'off');
        ...
        $scheduler_data = get_option('chatondesk_abandoned_cart_scheduler');
        $quiet_hours = chatondesk_get_option('enable_quiet_hours', 'chatondesk_abandoned_cart', '0');
        $form_quiet_hours = chatondesk_get_option('from_quiet_hours', 'chatondesk_abandoned_cart', '22:00');
        $to_quiet_hours = chatondesk_get_option('to_quiet_hours', 'chatondesk_abandoned_cart', '08:00');
        if ('1' === $quiet_hours) {
    ...
    function clearCartData( $order_id = null )
    {
        ...
        if (isset(WC()->session) ) {
            ...
            if (isset($cart_session_id) ) {
                $public        = new SA_Cart_Public(CHATONDESK_PLUGIN_NAME_SLUG, \ChatOnDesk\SmsAlertConstants::SA_VERSION);
    ...
    public static function restoreCart()
    {
        ...
        $main_table = $wpdb->prefix . COD_CART_TABLE_NAME;
        $row        = $wpdb->get_row(
            $wpdb->prepare(
                ...
                if (! $product_exists ) {
                    $this->log(
                        ...
                        /* translators: %d - Product ID */
                            __('Unable to restore product in the shopping cart since the product no longer exists. ID: %d', 'chat-on-desk'),
                            $product['product_id']
                        )
                    );
                } else {
...
class SA_Admin_Table extends \WP_List_Table
{
    ...
    function get_columns()
    {
        ...
        'id'            => __('ID', 'chat-on-desk'),
        'name'          => __('Name, Surname', 'chat-on-desk'),
        'email'         => __('Email', 'chat-on-desk'),
        'phone'         => __('Phone', 'chat-on-desk'),
        'location'      => __('Location', 'chat-on-desk'),
        'cart_contents' => __('Cart contents', 'chat-on-desk'),
        'cart_total'    => __('Cart total', 'chat-on-desk'),
        'time'          => __('Time', 'chat-on-desk'),
        'status'        => __('Status', 'chat-on-desk'),
        );
    }
    ...
    function column_name( $item )
    {
        ...
        $actions  = array(
        'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $req_page, $item['id'], __('Delete', 'chat-on-desk')),
        );
    ...
    function column_status( $item )
    {
        ...
        if ($cart_time > $current_time - CART_STILL_SHOPPING * 60 && '0' === $item['msg_sent'] && '0' === $item['recovered'] ) { // Checking time if user is still shopping or might return - we add shopping label
            $status .= sprintf('<span class="status shopping">%s</span>', __('Shopping', 'chat-on-desk'));
            ...
            if ($cart_time > ( $current_time - CART_NEW_STATUS_NOTICE * 60 ) && '0' === $item['msg_sent'] && '0' === $item['recovered'] ) { // Checking time if user has not gone through with the checkout after the specified time we add new label
                $status .= sprintf('<span class="status new" >%s</span>', __('New', 'chat-on-desk'));
            }
            if ('0' !== $item['msg_sent'] && '0' === $item['recovered'] ) {
                $status .= sprintf('<div class="status-item-container"><span class="status msg-sent" >%s (%s)</span></div>', __('MSG Sent', 'chat-on-desk'), $item['msg_sent']);
            }
            if ('1' === $item['recovered'] ) {
                $status .= sprintf('<div class="status-item-container"><span class="status recovered" >%s</span></div>', __('Recovered', 'chat-on-desk'));
            }
        }
        return $status;
    }
    ...
    {
        global $wpdb;
        $table_name = $wpdb->prefix . COD_CART_TABLE_NAME; // do not forget about tables prefix
		$verify = !empty($_REQUEST['_wpnonce'])?wp_verify_nonce($_REQUEST['_wpnonce'], 'bulk-' . $this->_args['plural']):false;
		if ($verify) {
			if ('delete' === $this->current_action() ) {
				$ids = isset($_REQUEST['id']) ? chatondesk_sanitize_array($_REQUEST['id']) : array();
				if (! empty($ids) ) {
...
class SA_Cart_Public
{
    ...
    public function enqueueStyles()
    {
        if ($this->exitIntentEnabled() ) { // If Exit Intent Enabled
            wp_enqueue_style($this->plugin_name, COD_MOV_URL . 'css/ab-public.css', array(), \ChatOnDesk\SmsAlertConstants::SA_VERSION, 'all');
        }
    }
    ...
    public function enqueueScripts()
    {
        if ($this->exitIntentEnabled() ) { // If Exit Intent Enabled
            ...
            if (is_user_logged_in() ) {
                ...
                $plugin_admin   = new SA_Cart_Admin(CHATONDESK_PLUGIN_NAME_SLUG, \ChatOnDesk\SmsAlertConstants::SA_VERSION);
            }
            ...
            if (chatondesk_get_option('cart_exit_intent_status', 'chatondesk_abandoned_cart', '0') ) {
                $data = array(
                'hours'             => 1,
                ...
                wp_enqueue_script($this->plugin_name . 'exit_intent', COD_MOV_URL . 'js/ab-public-exit-intent.js', array( 'jquery' ), \ChatOnDesk\SmsAlertConstants::SA_VERSION, false);
    ...
    public function addAdditionalScriptsOnCheckout()
    {
        ...
        $user_settings_notification_frequency = chatondesk_get_option('customer_notify', 'chatondesk_abandoned_cart', 'off');
        ...
        wp_enqueue_script($this->plugin_name, COD_MOV_URL . 'js/ab-cart-public.js', array( 'jquery' ), \ChatOnDesk\SmsAlertConstants::SA_VERSION, false);
    ...
    function saveUserData()
    {
        ...
        check_ajax_referer('chatondesk_wp_abcart_nonce', 'chatondesk_abcart_nonce');
        if (isset($_POST['ab_cart_phone']) ) {
            ...
            $phone = \ChatOnDesk\SmsAlertUtility::formatNumberForCountryCode($phone);
            $other_fields = array(
    ...
    function exitIntentEnabled()
    {
        ...
        $exit_intent_on        = chatondesk_get_option('cart_exit_intent_status', 'chatondesk_abandoned_cart', '0');
        $test_mode_on          = chatondesk_get_option('cart_exit_intent_test_mode', 'chatondesk_abandoned_cart', '0');
        $current_user_is_admin = current_user_can('manage_options');
