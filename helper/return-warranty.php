...
class Sa_Return_Warranty
{
    ...
    {
        $options = $_POST;
        if ('chatondesk_warranty' === $options['tab'] ) {
    ...
    {
        $wc_warranty_checkbox = chatondesk_get_option('warranty_status_' . $status, 'chatondesk_warranty', '');
        $is_sms_enabled       = ( 'on' === $wc_warranty_checkbox ) ? true : false;
        if ($is_sms_enabled ) {
            $sms_content = chatondesk_get_option('sms_text_' . $status, 'chatondesk_warranty', '');
            ...
            do_action('cod_send_sms', $buyer_mob, $message);
        }
    }
    ...
    public static function smsalertWarrantyTab()
    {
        ...
        ?>
        <a href="admin.php?page=warranties-settings&tab=chatondesk_warranty" class="nav-tab <?php echo ( 'chatondesk_warranty' === $active_tab ) ? 'nav-tab-active' : ''; ?>"><?php esc_attr_e('Chat On Desk', 'wc_warranty'); ?></a>
        <?php
    }
    ...
    public static function smsalertWarrantySettingsPanels()
    {
        ...
        if ('chatondesk_warranty' === $active_tab ) {
            $return_warranty_param = array(
            'checkTemplateFor' => 'return_warranty',
            'templates'        => self::getReturnWarrantyTemplates(),
            );
            ChatOnDesk\get_chatondesk_template('views/message-template.php', $return_warranty_param);
        }
    }
    ...
    public static function add_default_setting( $defaults = array() )
    {
        ...
        foreach ( $wc_warrant_status as $ks                           => $vs ) {
            ...
            $defaults['chatondesk_warranty'][ 'warranty_status_' . $vs ] = 'off';
            $defaults['chatondesk_warranty']['sms_text_'][ $vs ]         = '';
        }
        return $defaults;
    }
    ...
    public static function getReturnWarrantyTemplates()
    {
        ...
        foreach ( $wc_warrant_status as $ks                           => $vs ) {
            ...
            $vs               = str_replace(' ', '-', strtolower($vs));
            $wc_warranty_text = chatondesk_get_option('sms_text_' . $vs, 'chatondesk_warranty', '');
            $current_val      = chatondesk_get_option('warranty_status_' . $vs, 'chatondesk_warranty', 'on');

            $checkbox_name_id  = 'chatondesk_warranty[warranty_status_' . $vs . ']';
            $text_area_name_id = 'chatondesk_warranty[sms_text_' . $vs . ']';

            $text_body = chatondesk_get_option('sms_text_' . $vs, 'chatondesk_warranty', '') ? chatondesk_get_option('sms_text_' . $vs, 'chatondesk_warranty', '') : SmsAlertMessages::showMessage('DEFAULT_WARRANTY_STATUS_CHANGED');
