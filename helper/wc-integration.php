...
class SAShipmentIntegration
{
    ...
    public function __construct()
    {
        ...
        add_filter('cod_wc_order_sms_customer_before_send', array( $this, 'replaceAftershipTrackingno' ), 10, 2);
        add_filter('cod_wc_order_sms_customer_before_send', array( $this, 'replaceWcshipmentTrackingno' ), 10, 2);
        add_filter('cod_wc_order_sms_customer_before_send', array( $this, 'replaceWcAdvshipmentTrackingno' ), 10, 2);
        add_filter('cod_wc_order_sms_admin_before_send', array( $this, 'replaceWcAdvshipmentTrackingno' ), 10, 2);

        add_filter('cod_wc_variables', array( $this, 'addTokensWcTemplates' ), 10, 2);
    }
    ...
    public function replaceWcAdvshipmentTrackingno( $sms_data, $order_id )
    {
        if (is_plugin_active('woo-advanced-shipment-tracking/woocommerce-advanced-shipment-tracking.php')
            ...
            if (preg_match_all('/\[date_shipped.*?\]/', $content, $matched) ) {
                ...
                $date_params     = \ChatOnDesk\SmsAlertUtility::parseAttributesFromTag($date_shipped);
    ...
    public function triggerOrderTrackship( $order_id )
    {
        ...
        if (in_array($order_status, $order_status_settings, true)  ) {
            ...
            $buyer_sms_body             = chatondesk_get_option('sms_body_' . $order_status, 'chatondesk_message', $default_buyer_sms);
            ...
            do_action('cod_send_sms', $buyer_no, $buyer_sms_data['sms_body']);
        }
    }
}
...
class SAWCInvoicePdf
{
    ...
    public function __construct()
    {
        add_filter('cod_wc_order_sms_customer_before_send', array( $this, 'replaceTokenWcTemplates' ), 10, 2);
        add_filter('cod_wc_variables', array( $this, 'addTokensWcTemplates' ), 10, 2);
    }
...
class SAWCAuctions
{
    ...
    {
        $defaults['chatondesk_wcauction_general']['wcauction_admin_notification_new']          = 'off';
        $defaults['chatondesk_wcauction_general']['wcauction_bidder_notification_outbid']      = 'off';
        $defaults['chatondesk_wcauction_general']['wcauction_bidder_notification_customerbid'] = 'off';
        $defaults['chatondesk_wcauction_message']['wcauction_admin_sms_body_new']              = '';
        $defaults['chatondesk_wcauction_message']['wcauction_sms_body_outbid']                 = '';
        $defaults['chatondesk_wcauction_message']['wcauction_sms_body_customerbid']            = '';
        return $defaults;
    ...
    public static function addTabs( $tabs = array() )
    {
        ...
        'checkTemplateFor' => 'cod_wc_auction',
        'templates'        => self::getCustomerTemplates(),
        );

        $admin_param = array(
        'checkTemplateFor' => 'cod_wc_auction_admin',
        'templates'        => self::getAdminTemplates(),
        );

        $tabs['cod_wc_auction']['nav']  = 'Woo Product Auction';
        $tabs['cod_wc_auction']['icon'] = 'dashicons-admin-users';

        $tabs['cod_wc_auction']['inner_nav']['wc_auction_customer']['title']        = 'Customer Notifications';
        $tabs['cod_wc_auction']['inner_nav']['wc_auction_customer']['tab_section']  = 'wcauctioncsttemplates';
        $tabs['cod_wc_auction']['inner_nav']['wc_auction_customer']['first_active'] = true;
        $tabs['cod_wc_auction']['inner_nav']['wc_auction_customer']['tabContent']   = $customer_param;
        $tabs['cod_wc_auction']['inner_nav']['wc_auction_customer']['filePath']     = 'views/message-template.php';

        $tabs['cod_wc_auction']['inner_nav']['wc_auction_admin']['title']       = 'Admin Notifications';
        $tabs['cod_wc_auction']['inner_nav']['wc_auction_admin']['tab_section'] = 'wcauctionadmintemplates';
        $tabs['cod_wc_auction']['inner_nav']['wc_auction_admin']['tabContent']  = $admin_param;
        $tabs['cod_wc_auction']['inner_nav']['wc_auction_admin']['filePath']    = 'views/message-template.php';
        return $tabs;
    }
    ...
    public static function getCustomerTemplates()
    {
        ...
        $templates['outbid']['enabled']        = chatondesk_get_option('wcauction_bidder_notification_outbid', 'chatondesk_wcauction_general', 'on');
        $templates['outbid']['status']         = 'outbid';
        $templates['outbid']['text-body']      = chatondesk_get_option('wcauction_sms_body_outbid', 'chatondesk_wcauction_message', sprintf('Hello %1$s, a new bid for auction %2$s has just been submitted. The new bid is: %3$s. Please visit the auction %4$s', '[first_name]', '[auction_name]', '[auction_bid]', '[auction_link]'));
        $templates['outbid']['checkboxNameId'] = 'chatondesk_wcauction_general[wcauction_bidder_notification_outbid]';
        $templates['outbid']['textareaNameId'] = 'chatondesk_wcauction_message[wcauction_sms_body_outbid]';
        $templates['outbid']['token']          = self::getvariables();
        /*Send SMS to Bidder*/
        $templates['customerbid']['title']   = 'Send SMS to Bidder';
        $templates['customerbid']['enabled'] = chatondesk_get_option('wcauction_bidder_notification_customerbid', 'chatondesk_wcauction_general', 'on');
        $templates['customerbid']['status']  = 'customerbid';

        $templates['customerbid']['text-body']      = chatondesk_get_option('wcauction_sms_body_customerbid', 'chatondesk_wcauction_message', sprintf('Hello %1$s, Thank You for placing bid for %2$s. Your bid is %3$s. Please visit the auction %4$s', '[first_name]', '[auction_name]', '[auction_bid]', '[auction_link]'));
        $templates['customerbid']['checkboxNameId'] = 'chatondesk_wcauction_general[wcauction_bidder_notification_customerbid]';
        $templates['customerbid']['textareaNameId'] = 'chatondesk_wcauction_message[wcauction_sms_body_customerbid]';
    ...
    public static function getAdminTemplates()
    {
        $templates   = array();
        $ks          = 'new';
        $current_val = chatondesk_get_option('wcauction_admin_notification_new', 'chatondesk_wcauction_general', 'on');

        $checkbox_name_id  = 'chatondesk_wcauction_general[wcauction_admin_notification_new]';
        $text_area_name_id = 'chatondesk_wcauction_message[wcauction_admin_sms_body_new]';

        $text_body = chatondesk_get_option('wcauction_admin_sms_body_new', 'chatondesk_wcauction_message', sprintf('%1$s a new bid for auction %2$s has been submitted by %3$s. The new bid is: %4$s. Please visit the auction %5$s', '[store_name]:', '[auction_name]', '[first_name]', '[auction_bid]', '[auction_link]'));
    ...
    {
        $outbid  = chatondesk_get_option('wcauction_bidder_notification_outbid', 'chatondesk_wcauction_general');
        $message = chatondesk_get_option('wcauction_sms_body_outbid', 'chatondesk_wcauction_message');
        ...
        if ('on' === $outbid && '' !== $message ) {
            ...
            do_action('cod_send_sms', $outbider_phone, $this->replaceTokenWcTemplates($message, $product_id, $outbiddeduser_id));
        }
    }
    ...
    {
        $customerbid = chatondesk_get_option('wcauction_bidder_notification_customerbid', 'chatondesk_wcauction_general');
        $message     = chatondesk_get_option('wcauction_sms_body_customerbid', 'chatondesk_wcauction_message');
        ...
        if ('on' === $customerbid && '' !== $message ) {
            ...
            do_action('cod_send_sms', $cur_bidder_phone, $this->replaceTokenWcTemplates($message, $product_id, $current_bidder_id));
        }
    }
    ...
    {
        $admin_outbid      = chatondesk_get_option('wcauction_admin_notification_new', 'chatondesk_wcauction_general');
        $admin_sms_content = chatondesk_get_option('wcauction_admin_sms_body_new', 'chatondesk_wcauction_message');
        ...
        if ('on' === $admin_outbid && '' !== $admin_phone_number && '' !== $admin_sms_content ) {
            ...
            $product_data       = wc_get_product($product_id);
            $current_bidder_id  = $product_data->get_auction_current_bider();
            do_action('cod_send_sms', $admin_phone_number, $this->replaceTokenWcTemplates($admin_sms_content, $product_id, $current_bidder_id));
        }
    }
...
class SAWCDokan
{
    ...
    public function __construct()
    {
        add_filter('cod_post_author_no', array( $this, 'setVendorPhoneNo' ), 10, 1);
    ...
    public function sendApprovedSmsVendor( $user_id )
    {
        ...
        $enabled = chatondesk_get_option('multivendor_notification_approved', 'chatondesk_general');
        if ('on' === $enabled && ! empty($author_no) ) {
            $content = chatondesk_get_option('multivendor_sms_body_approved', 'chatondesk_message', SmsAlertMessages::showMessage('DEFAULT_NEW_USER_APPROVED'));
            do_action('cod_send_sms', $author_no, $this->parseSmsBody($content, $user_id));
        }
    }
    ...
    public function sendRejectedSmsVendor( $user_id )
    {
        ...
        $enabled = chatondesk_get_option('multivendor_notification_rejected', 'chatondesk_general');
        if ('on' === $enabled && ! empty($author_no) ) {
            $content = chatondesk_get_option('multivendor_sms_body_rejected', 'chatondesk_message', SmsAlertMessages::showMessage('DEFAULT_NEW_USER_REJECTED'));
            do_action('cod_send_sms', $author_no, $this->parseSmsBody($content, $user_id));
        }
    }
...
class SAWCFM
{
    ...
    public function activateVendor( $member_id )
    {
        ...
        if ('on' === $enabled && ! empty($author_no) ) {
            $content = chatondesk_get_option('multivendor_sms_body_approved', 'chatondesk_message', SmsAlertMessages::showMessage('DEFAULT_NEW_USER_APPROVED'));
            do_action('cod_send_sms', $author_no, $this->parseSmsBody($content, $member_id));
        }
    }
...
class SAWCLocalPickup
{
    ...
    public function __construct()
    {
        add_filter('cod_store_manager_no', array( $this, 'setVendorPhoneNo' ), 10, 1);
    }
...
class SADigit
{
    ...
    public function __construct()
    {
        add_filter('cod_get_user_phone_no', array( $this, 'setUserPhoneNo' ), 10, 2);
    }
...
class SAReminderlist
{
    ...
    public static function reminderCartCount()
    {
        ...
        $source=isset($_REQUEST['source'])?$_REQUEST['source']:'';
            $table_name  = $wpdb->prefix . 'chatondesk_booking_reminder';
        ...
        ?>
            ...
            if ('admin.php' === $pagenow && 'booking-reminder' === $_GET['page'] ) {
                ...
                if ('0' === self::reminderCartCount() ) { 
                    ...
                    ?>
                                <p>
                            <?php esc_html_e('Looks like you do not have any booking reminder.', 'chat-on-desk'); ?>
                                </p>
                <?php } else { ?>
                                <form method="GET">
...
class SA_Admin_Reminder_Table extends \WP_List_Table
{
    ...
    function get_columns()
    {
            return $columns = array(
                    ...
                    'booking_id'    => __('Booking Id', 'chat-on-desk'),
                    'phone'         => __('Phone', 'chat-on-desk'),
                    'start_date'    => __('Date', 'chat-on-desk'),
                    'msg_sent'      => __('Status', 'chat-on-desk'),
            );
    }
        /**
    ...
    function column_status( $item )
    {
        ...
        return sprintf('<div class="status-item-container"><span class="status msg-sent" >%s (%s)</span></div>', __('MSG Sent', 'chat-on-desk'), $item['msg_sent']);
    }
        /**
         * Column cb function.
         *
    ...
    {
            global $wpdb;
            $table_name = $wpdb->prefix . 'chatondesk_booking_reminder'; // do not forget about tables prefix
