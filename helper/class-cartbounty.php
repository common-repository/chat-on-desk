...
class WCAbandonedCart
{
    ...
    {
        $defaults['chatondesk_ac_general']['customer_notify'] = 'off';
        $defaults['chatondesk_ac_message']['customer_notify'] = '';
        $defaults['chatondesk_ac_general']['admin_notify']    = 'off';
        $defaults['chatondesk_ac_message']['admin_notify']    = '';
        return $defaults;
    }
    ...
    {
        // customer template.
        $current_val      = chatondesk_get_option('customer_notify', 'chatondesk_ac_general', 'on');
        $checkbox_name_id = 'chatondesk_ac_general[customer_notify]';
        $textarea_name_id = 'chatondesk_ac_message[customer_notify]';
        $text_body        = chatondesk_get_option('customer_notify', 'chatondesk_ac_message', SmsAlertMessages::showMessage('DEFAULT_AC_CUSTOMER_MESSAGE'));
        ...
        // admin template.
        $current_val      = chatondesk_get_option('admin_notify', 'chatondesk_ac_general', 'on');
        $checkbox_name_id = 'chatondesk_ac_general[admin_notify]';
        $textarea_name_id = 'chatondesk_ac_message[admin_notify]';
        $text_body        = chatondesk_get_option('admin_notify', 'chatondesk_ac_message', SmsAlertMessages::showMessage('DEFAULT_AC_ADMIN_MESSAGE'));
    ...
    public function smsalertSendSms()
    {
        ...
        if (is_plugin_active('woo-save-abandoned-carts-pro/cartbounty-pro-abandoned-carts.php') ) {
            ...
            $time             = chatondesk_get_option('hours', 'cartbounty_pro_notification_frequency', '60');
            ...
            $time       = chatondesk_get_option('hours', 'cartbounty_notification_frequency', '60');
        }
        ...
        if ($rows_to_phone ) {
            $chatondesk_ac_customer_notify  = chatondesk_get_option('customer_notify', 'chatondesk_ac_general', 'on');
            $chatondesk_ac_customer_message = chatondesk_get_option('customer_notify', 'chatondesk_ac_message', '');

            if ('on' === $chatondesk_ac_customer_notify && '' !== $chatondesk_ac_customer_message ) {
                foreach ( $rows_to_phone as $data ) {
                    if ($set_checkout_url ) {
                        $data['checkout_url'] = $plugin_admin->create_cart_url($data['email'], $data['session_id'], $data['id']);
                    }
                    do_action('cod_send_sms', $data['phone'], $this->parseSmsBody($data, $chatondesk_ac_customer_message));
                }
            }
            ...
            if (! empty($sms_admin_phone) ) {
                $chatondesk_ac_admin_notify  = chatondesk_get_option('admin_notify', 'chatondesk_ac_general', 'on');
                $chatondesk_ac_admin_message = chatondesk_get_option('admin_notify', 'chatondesk_ac_message', '');

                if ('on' === $chatondesk_ac_admin_notify && '' !== $chatondesk_ac_admin_message ) {
                    $sms_admin_phone = explode(',', $sms_admin_phone);
                    foreach ( $sms_admin_phone as $phone ) {
                        do_action('cod_send_sms', $phone, $this->parseSmsBody($data, $chatondesk_ac_admin_message));
                    }
                }
            }
        }
