...
class Sa_Backinstock
{
    ...
    public function __construct()
    {
        ...
        $chatondesk_bis_subscribed_notify = chatondesk_get_option('subscribed_bis_notify', 'chatondesk_bis_general', 'on');

        if ('on' === $chatondesk_bis_subscribed_notify ) {
        ...
        add_action('woocommerce_update_product', array($this, 'triggerOnProductStockStatusChanged'), 10, 1); //add on  11-10-21
    }
    ...
    public function showSubscriberInVariationProduct( $loop, $variation_data, $variation )
    {
		...
		<label for="chatondesk_backinstock_subscribers">' . __('Backorders', 'chat-on-desk') . '</label>
		<input type="text" name="chatondesk_backinstock_subscribers" value="' . All_Subscriber_List::getNosSubscribersByProductId($variation->ID) . '" class="input-text short chatondesk_backinstock_subscribers" disabled style="border:none;box-shadow:none"/>
	</p>',
            $allowedposttags
        );
    }
    ...
    public function showSubscriberInSingleProduct()
    {
        ...
        if ('variable' !== $product_object->get_type() ) {
            ...
            echo wp_kses(
                '<p class="form-field">
            <label for="chatondesk_backinstock_subscribers">' . esc_html__('Backorders', 'chat-on-desk') . '</label>
			<input type="text" name="chatondesk_backinstock_subscribers" value="' . All_Subscriber_List::getNosSubscribersByProductId($post->ID) . '" class="input-text short chatondesk_backinstock_subscribers" disabled style="border:none;box-shadow:none"/>
        </p>',
                $allowedposttags
            );
        }
    ...
    public function enqueueScriptOnPage()
    {
        ...
        wp_register_script('cod_single_product', COD_MOV_URL . 'js/wc-product.js', array( 'jquery' ), \ChatOnDesk\SmsAlertConstants::SA_VERSION, true);
        ...
        wp_localize_script(
            ...
            'cod_default_countrycode',
            chatondesk_get_option('default_country_code', 'chatondesk_general')
        );
        ...
        wp_localize_script(
            ...
            'cod_notices',
            array(
            'waiting_txt' => __('Please wait...', 'chat-on-desk'),
            'enter_here'  => __('Enter Number Here', 'chat-on-desk'),
            )
        );
        wp_enqueue_script('cod_single_product');
    }
    ...
    public static function getBackInStockTemplates()
    {
        ...
        $current_val      = chatondesk_get_option('customer_bis_notify', 'chatondesk_bis_general', 'on');
        $checkbox_name_id = 'chatondesk_bis_general[customer_bis_notify]';
        $textarea_name_id = 'chatondesk_bis_message[customer_bis_notify]';
        $text_body        = chatondesk_get_option(
            'customer_bis_notify',
            'chatondesk_bis_message',
        ...
        // product subscribed.
        $current_val      = chatondesk_get_option('subscribed_bis_notify', 'chatondesk_bis_general', 'on');
        $checkbox_name_id = 'chatondesk_bis_general[subscribed_bis_notify]';
        $textarea_name_id = 'chatondesk_bis_message[subscribed_bis_notify]';
        $text_body        = chatondesk_get_option(
    ...
    public function processSmsForInStockSubscribers( $product_id, $product_status )
    {
        ...
        $datas        = $wpdb->get_results("SELECT * FROM {$table_prefix}postmeta WHERE meta_key = 'chatondesk_instock_pid' and meta_value = '$product_id'", ARRAY_A);
        $obj          = array();
        $posts        = array();
        foreach ( $datas as $dkey => $data ) {

            $post_id   = $data['post_id'];
            $post_data = $wpdb->get_results("SELECT ID , post_title, post_author FROM {$table_prefix}posts WHERE post_status = 'cod_subscribed' and ID = '$post_id'", ARRAY_A);

            $chatondesk_bis_cust_notify = chatondesk_get_option('customer_bis_notify', 'chatondesk_bis_general', 'on');
            if (! empty($post_data) && 'instock' === $product_status && 'on' === $chatondesk_bis_cust_notify ) {
				$post_user_id             = $post_data[0]['post_author'];
                $posts[ $dkey ]['post_id'] = $post_data[0]['ID'];
                $backinstock_message       = chatondesk_get_option('customer_bis_notify', 'chatondesk_bis_message', '');
    ...
    {
        if (! empty($data) ) {
            if (isset($data['action']) && 'chatondeskbackinstock' === $data['action'] ) {
    ...
    {
        $defaults['chatondesk_bis_general']['customer_bis_notify']   = 'off';
        $defaults['chatondesk_bis_message']['customer_bis_notify']   = '';
        $defaults['chatondesk_bis_general']['subscribed_bis_notify'] = 'off';
        $defaults['chatondesk_bis_message']['subscribed_bis_notify'] = '';
        return $defaults;
    }
    ...
    public function displaySaSubscribeBox( $product, $variation = array() )
    {
        ...
        $get_option              = get_option('chatondesk_instocksettings');
        ...
        if ($check_guest_visibility && $check_member_visibility && 'instock' !== $product_status && ! $variation_id ) {
            ...
            return get_chatondesk_template('template/backinstock-template.php', $params, true);
    ...
    public function performActionOnAjaxData( $post_data )
    {
        ...
        $subscriber_phone = Chatondesk::checkPhoneNos($user_phone);
        if (! $subscriber_phone ) {
            $data['status']      = 'error';
            $data['description'] = str_replace('##phone##', $user_phone, $phoneCodLogic->_get_otp_invalid_format_message());
        } else {
            $get_user_id  = $user_id;
            ...
            if (! empty($check_is_already_subscribed) ) {
                $data['status']      = 'error';
                $data['description'] = __('Seems like you have already subscribed to this product', 'chat-on-desk');
            } else {
                if ('' !== $subscriber_phone ) {
                    ...
                    $default_data = array(
                    'chatondesk_instock_variation_id' => $variation_id,
                    'chatondesk_subscriber_phone'     => $subscriber_phone,
                    'chatondesk_instock_user_id'      => $get_user_id,
                    'chatondesk_instock_pid'          => $product_id,
                    );
                    foreach ( $default_data as $key => $value ) {
                        update_post_meta($post_id, $key, $value);
                    }

                    $subscribed_bis_notify = chatondesk_get_option('subscribed_bis_notify', 'chatondesk_bis_general', '');
                    $subscribed_message       = chatondesk_get_option('subscribed_bis_notify', 'chatondesk_bis_message', '');
                    ...
                    if ('on' === $subscribed_bis_notify && '' !== $subscribed_message ) {
                        ...
                        Chatondesk::sendsms($buyer_sms_data);
                    }
                    $data['status']      = 'success';
                    $data['description'] = __('You have subscribed successfully.', 'chat-on-desk');
                }
            }
        }
    ...
    public function insertSubscriber( $mobileno, $user_id )
    {
        $args = array(
        'post_title'  => $mobileno,
        'post_type'   => 'codinstocknotifier',
        'post_status' => 'cod_subscribed',
        'post_author' => $user_id,
        );
		global $wp_rewrite;
    ...
    public function isAlreadySubscribed( $product_id, $variation_id, $subscriber_phone, $get_user_id )
    {
        ...
        $wcc_ph     = Chatondesk::checkPhoneNos($subscriber_phone);
        $wocc_ph    = Chatondesk::checkPhoneNos($subscriber_phone, false);
        $wth_pls_ph = '+' . $wcc_ph;

        $table_prefix = $wpdb->prefix;
        $product_id   = ( $variation_id > '0' || $variation_id > 0 ) ? $variation_id : $product_id;
        $datas        = $wpdb->get_results("SELECT * FROM {$table_prefix}postmeta pm1 inner join {$table_prefix}postmeta pm2 on pm1.post_id= pm2.post_id WHERE pm1.meta_key = 'chatondesk_instock_pid' and pm1.meta_value = '$product_id' and pm2.meta_key ='chatondesk_subscriber_phone' and pm2.meta_value in('$wcc_ph','$wocc_ph','$wth_pls_ph')", ARRAY_A);
        ...
        if (! empty($post_ids) ) {
            $post_data = $wpdb->get_results("SELECT ID,post_title, post_status FROM {$table_prefix}posts WHERE post_status = 'cod_subscribed' and ID in (" . implode(',', $post_ids) . ')', ARRAY_A);
        }
        return $post_data;
    }
...
class All_Subscriber_List extends \WP_List_Table
{
    ...
    public static function getAllSubscriber( $per_page = 5, $page_number = 1 )
    {
        ...
        $sql = "SELECT P.ID, P.post_author, P.post_title, P.post_status,P.post_content, PM.meta_value FROM {$wpdb->prefix}posts P inner join {$wpdb->prefix}postmeta PM on P.ID = PM.post_id WHERE P.post_type = 'codinstocknotifier' and PM.meta_key = 'chatondesk_instock_pid'";
    ...
    {
        global $wpdb;
        $sql    = "SELECT count(*) as cnt FROM {$wpdb->prefix}posts P inner join {$wpdb->prefix}postmeta PM on P.ID = PM.post_id WHERE P.post_type = 'codinstocknotifier' and PM.meta_key = 'chatondesk_instock_pid' and P.post_status = 'cod_subscribed'";
    ...
    {
        esc_html_e('No Subscriber.', 'chat-on-desk');
    }
    ...
    {
        if ('cod_subscribed' === $item['post_status'] ) {
            $post_status = '<button class="button-primary"/>Subscribed</a>';
        } elseif ('cod_general_error' === $item['post_status'] ) {
    ...
    public static function recordCount()
    {
        ...
        $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}posts where post_type = 'codinstocknotifier'";
...
{
    add_submenu_page('options.php', 'All Subscriber', 'All Subscriber', 'manage_options', 'all-cod-subscriber', 'ChatOnDesk\subscriberPageHandler');
}

add_action('admin_menu', 'ChatOnDesk\allSubscriberAdminMenu');
