...
class SmsAlertEdd
{
    ...
    public function __construct()
    {
        ...
        add_action('edd_settings_tab_bottom_chatondesk_customer_notification', array($this, 'getCustomerTemplate' ), 10);
        add_action('edd_settings_tab_bottom_chatondesk_admin_notification', array($this, 'getAdminTemplate' ), 10);
    }
    ...
    public function getCustomerTemplate()
    {
        ...
        foreach ( $edd_order_statuses as $ks  => $vs ) {
            ...
            $current_val = chatondesk_get_option('edd_order_status_' . $vs, 'edd_settings', 'off');
    ...
    public function getAdminTemplate()
    {
        ...
        $templates = array();
        foreach ( $edd_order_statuses as $ks  => $vs ) {
            $current_val       = chatondesk_get_option('edd_admin_notification_' . $vs, 'edd_settings', 'off');
    ...
    public function addSections($sections)
    {
        ...
        'customer_notification' => __('Customer Notifications', 'chat-on-desk'),
        'admin_notification'    => __('Admin Notifications', 'chat-on-desk')
        );
        return $sections;
    }
    ...
    public function addRegisterSettings($settings)
    {
        ...
        $settings['smsalert']= apply_filters(
            ...
            'id'   => 'customer_setting',
            'name' => '<h3>' . __('Customer', 'chat-on-desk') . '</h3>',
            ...
            'admin_notification' => array(
                    'admin_setting' => array(
                        'id'   => 'admin_setting',
                        'name' => '<h3>' . __('Admin', 'chat-on-desk') . '</h3>',
                        'type' => 'descriptive_text'
            )
            )
            )            
    ...
    public static function triggerAfterUpdateEddStatus( $payment_id )
    {
        ...
        $admin_send = chatondesk_get_option('edd_admin_notification_' . $status, 'edd_settings');
        $cst_send   = chatondesk_get_option('edd_order_status_' . $status, 'edd_settings');

        if ('on' === $cst_send ) {
            $content = chatondesk_get_option('edd_sms_body_' . $status, 'edd_settings');
        ...
        if ('on' === $admin_send ) {
            ...
            $content = chatondesk_get_option('edd_admin_sms_body_' . $status, 'edd_settings');
