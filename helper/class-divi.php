...
class SMSAlertDivi
{
    ...
    public function hooks()
    {
        ...
        if (wp_doing_ajax() ) {
            add_action('wp_ajax_chatondesk_divi_preview', [ $this, 'preview' ]);
        }
    ...
    {
        wp_enqueue_script('chatondesk-divi', COD_MOV_URL . 'js/divi.js', [ 'react', 'react-dom' ], \ChatOnDesk\SmsAlertConstants::SA_VERSION, true);

        wp_localize_script(
            'chatondesk-divi',
            'chatondesk_divi_builder',
            [
            'ajax_url'          => admin_url('admin-ajax.php'),
            'nonce'             => wp_create_nonce('chatondesk_divi_builder'),
            'placeholder'       => '',
            'placeholder_title' => esc_html__('ChatOnDesk', 'chat-on-desk'),
            ]
        );
    }
    ...
    public function preview()
    {
        ...
        check_ajax_referer('chatondesk_divi_builder', 'nonce');

        $form_id    = filter_input(INPUT_POST, 'form_id', FILTER_SANITIZE_STRING);

        add_action(
            'chatondesk_frontend_output',
            function () {
        ...
        if ($form_id!='') {
              $shortcode = ($form_id==1)?'[cod_signupwithmobile]':(($form_id==2)?'[cod_loginwithotp]':'[cod_sharecart]');
