...
class SmsAlertWcBooking
{
    ...
    public static function setBookingReminder( $booking_id )
    {
        ...
        $customer_notify = chatondesk_get_option('customer_notify', 'chatondesk_wcbk_general', 'on');
        global $wpdb;
    ...
    {
        if ('on' !== chatondesk_get_option('customer_notify', 'chatondesk_wcbk_general') ) {
            return;
        }
        ...
        $table_name     = $wpdb->prefix . 'chatondesk_booking_reminder';
        $source = 'woocommerce-bookings';
        $scheduler_data = get_option('chatondesk_wcbk_reminder_scheduler');
    ...
    public static function getCustomerTemplates()
    {
        ...
        foreach ( $wcbk_order_statuses as $ks  => $vs ) {
            ...
            $current_val = chatondesk_get_option('wcbk_order_status_' . $vs, 'chatondesk_wcbk_general', 'on');

            $check_box_name_id = 'chatondesk_wcbk_general[wcbk_order_status_' . $vs . ']';
            $text_area_name_id = 'chatondesk_wcbk_message[wcbk_sms_body_' . $vs . ']';

            $text_body = chatondesk_get_option('wcbk_sms_body_' . $vs, 'chatondesk_wcbk_message', sprintf('Hello %1$s, status of your booking %2$s with %3$s has been changed to %4$s.', '[first_name]', '[booking_id]', '[store_name]', '[booking_status]'));
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ( $wcbk_order_statuses as $ks  => $vs ) {
            ...
            $current_val = chatondesk_get_option('wcbk_admin_notification_' . $vs, 'chatondesk_wcbk_general', 'on');

            $check_box_name_id = 'chatondesk_wcbk_general[wcbk_admin_notification_' . $vs . ']';
            $text_area_name_id = 'chatondesk_wcbk_message[wcbk_admin_sms_body_' . $vs . ']';

            $text_body = chatondesk_get_option('wcbk_admin_sms_body_' . $vs, 'chatondesk_wcbk_message', sprintf('%1$s status of order %2$s has been changed to %3$s.', '[store_name]:', '#[booking_id]', '[booking_status]'));
    ...
    {
        $current_val      = chatondesk_get_option('customer_notify', 'chatondesk_wcbk_general', 'on');
        $checkbox_name_id = 'chatondesk_wcbk_general[customer_notify]';
        ...
        foreach ( $scheduler_data['cron'] as $key => $data ) {
            ...
            $text_area_name_id = 'chatondesk_wcbk_reminder_scheduler[cron][' . $count . '][message]';
            $select_name_id    = 'chatondesk_wcbk_reminder_scheduler[cron][' . $count . '][frequency]';
    ...
    public static function add_default_setting( $defaults = array() )
    {
        ...
        foreach ( $wcbk_order_statuses as $ks => $vs ) {
            $defaults['chatondesk_wcbk_general'][ 'wcbk_admin_notification_' . $vs ] = 'off';
            $defaults['chatondesk_wcbk_general'][ 'wcbk_order_status_' . $vs ]       = 'off';
            $defaults['chatondesk_wcbk_message'][ 'wcbk_admin_sms_body_' . $vs ]     = '';
            $defaults['chatondesk_wcbk_message'][ 'wcbk_sms_body_' . $vs ]           = '';
        }
        $defaults['chatondesk_wcbk_general']['customer_notify']                = 'off';
        $defaults['chatondesk_wcbk_reminder_scheduler']['cron'][0]['frequency'] = '1';
        $defaults['chatondesk_wcbk_reminder_scheduler']['cron'][0]['message']   = '';
        return $defaults;
    }
    ...
    public static function triggerSms( $booking_id )
    {
        if ($booking_id ) {
            ...
            $booking_status = $object->status;
            $admin_message  = chatondesk_get_option('wcbk_admin_sms_body_' . $booking_status, 'chatondesk_wcbk_message', '');
            $is_enabled     = chatondesk_get_option('wcbk_order_status_' . $booking_status, 'chatondesk_wcbk_general');
            ...
            if ('' !== $buyer_mob && 'on' === $is_enabled ) {
                $buyer_message = chatondesk_get_option('wcbk_sms_body_' . $booking_status, 'chatondesk_wcbk_message', '');
                $content       = self::parseSmsBody($booking_id, $buyer_message);
                do_action('cod_send_sms', $buyer_mob, $content);
            }

            if ('on' === chatondesk_get_option('wcbk_admin_notification_' . $booking_status, 'chatondesk_wcbk_general') && '' !== $admin_phone_number ) {
                ...
                do_action('cod_send_sms', $admin_phone_number, $content);
            }
        }
    }
