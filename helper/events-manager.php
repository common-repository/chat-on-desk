...
class SmsAlertEMBooking
{
    ...
    public static function add_default_setting( $defaults = array() )
    {
        ...
        foreach ( $embk_booking_statuses as $ks => $vs ) {
            $defaults['chatondesk_embk_general'][ 'embk_admin_notification_' . $vs ] = 'off';
            $defaults['chatondesk_embk_general'][ 'embk_order_status_' . $vs ]       = 'off';
            $defaults['chatondesk_embk_message'][ 'embk_admin_sms_body_' . $vs ]     = '';
            $defaults['chatondesk_embk_message'][ 'embk_sms_body_' . $vs ]           = '';
        }
        return $defaults;
    }
    ...
    public static function getCustomerTemplates()
    {
        ...
        foreach ( $embk_booking_statuses as $ks  => $vs ) {
            ...
            $ks          = $vs;
            $ks          = str_replace(' ', '_', $ks);
            $current_val = chatondesk_get_option('embk_order_status_' . $vs, 'chatondesk_embk_general', 'on');

            $checkbox_name_id = 'chatondesk_embk_general[embk_order_status_' . $vs . ']';
            $textarea_name_id = 'chatondesk_embk_message[embk_sms_body_' . $vs . ']';

            $text_body = chatondesk_get_option(
                'embk_sms_body_' . $vs,
                'chatondesk_embk_message',
    ...
    public static function getAdminTemplates()
    {
        ...
        foreach ( $embk_booking_statuses as $ks  => $vs ) {
            ...
            $current_val = chatondesk_get_option('embk_admin_notification_' . $vs, 'chatondesk_embk_general', 'on');

            $checkbox_name_id = 'chatondesk_embk_general[embk_admin_notification_' . $vs . ']';
            $textarea_name_id = 'chatondesk_embk_message[embk_admin_sms_body_' . $vs . ']';
    ...
    public static function sendSmsWpbcBookingCreated( $booking )
    {
        if (function_exists('em_get_booking') ) {
            ...
            $is_enabled = chatondesk_get_option('embk_order_status_' . $booking_status[ $current_booking ], 'chatondesk_embk_general');
            if ('' !== $buyer_phone_number && 'on' === $is_enabled ) {
                $buyer_message = chatondesk_get_option('embk_sms_body_' . $booking_status[ $current_booking ], 'chatondesk_embk_message', '');

                $buyer_message = str_replace('[#_BOOKINGSTATUS]', $booking_status[ $current_booking ], $buyer_message);
                do_action('cod_send_sms', $buyer_phone_number, self::parseSmsBody($buyer_message, $booking));
            }
            if (chatondesk_get_option('embk_admin_notification_' . $booking_status[ $current_booking ], 'chatondesk_embk_general') === 'on' && '' !== $admin_phone_number ) {
                $admin_message = chatondesk_get_option('embk_admin_sms_body_' . $booking_status[ $current_booking ], 'chatondesk_embk_message', '');
                $admin_message = str_replace('[#_BOOKINGSTATUS]', $booking_status[ $current_booking ], $admin_message);
                do_action('cod_send_sms', $admin_phone_number, self::parseSmsBody($admin_message, $booking));
            }
        } else {
